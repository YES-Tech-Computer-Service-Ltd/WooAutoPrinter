...
class SettingsRepositoryImpl @Inject constructor(
        ...
        return dataStore.data
            .catch { exception ->
                if (exception is IOException) {
                    Log.e("SettingsRepositoryImpl", "Error reading selected_store_location.", exception)
            ...
            .map { preferences ->
                val json = preferences[PreferencesKeys.SELECTED_STORE_LOCATION]
                if (json.isNullOrBlank()) {
                    null
                } else {
                    try {
                        gson.fromJson(json, StoreLocationSelection::class.java)
                    } catch (e: Exception) {
                        Log.w("SettingsRepositoryImpl", "Failed to parse selected_store_location JSON: ${e.message}")
                        null
                    }
                }
            }
    ...
    override suspend fun setSelectedStoreLocation(selection: StoreLocationSelection?) {
        dataStore.edit { settings ->
            if (selection == null) {
                ...
                settings[PreferencesKeys.SELECTED_STORE_LOCATION] = gson.toJson(selection)
            }
        }
    }
    ...
    override suspend fun getSoundType(): String {
        val current = dataStore.data
            ...
            .map { preferences ->
                preferences[PreferencesKeys.SOUND_TYPE] ?: SoundSettings.SOUND_TYPE_BUILTIN_CHIME
            }.first()

        // 一次性迁移：如果是旧的系统类型，映射为内置清脆叮咚
        val legacySystemTypes = setOf(
        ...
        return if (legacySystemTypes.contains(current)) {
            try {
                dataStore.edit { settings ->
    ...
    // 亮度持久化
    override fun getAppBrightnessFlow(): Flow<Int?> {
        return dataStore.data
            .catch { exception ->
                if (exception is IOException) {
                    Log.e("SettingsRepositoryImpl", "Error reading app_brightness_percent.", exception)
                    emit(androidx.datastore.preferences.core.emptyPreferences())
                } else {
                    throw exception
                }
            ...
            .map { preferences ->
                preferences[PreferencesKeys.APP_BRIGHTNESS_PERCENT]
            }
    }
    ...
    override suspend fun setAppBrightness(percent: Int) {
        ...
        dataStore.edit { settings ->
            settings[PreferencesKeys.APP_BRIGHTNESS_PERCENT] = clamped
        }
    }

    override suspend fun clearAppBrightness() {
        dataStore.edit { settings ->
            settings.remove(PreferencesKeys.APP_BRIGHTNESS_PERCENT)
        }
    }
} 