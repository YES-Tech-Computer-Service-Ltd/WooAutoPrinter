...
object SslUtil {
     ...
     */
    fun configureForNougat(builder: OkHttpClient.Builder) {
        try {
            // 创建TrustManager
            ...
            // 使用标准SSLSocketFactory，同时提供TrustManager
            builder.sslSocketFactory(sc.socketFactory, trustManager)
            
            // 创建ConnectionSpec - 增加多个TLS版本支持
            val spec = ConnectionSpec.Builder(ConnectionSpec.MODERN_TLS)
                .tlsVersions(TlsVersion.TLS_1_2, TlsVersion.TLS_1_1, TlsVersion.TLS_1_0)
                .build()
            
            // 设置ConnectionSpecs，同时保留兼容模式
            builder.connectionSpecs(listOf(spec, ConnectionSpec.COMPATIBLE_TLS))
            
            Log.d(TAG, "成功配置Android 7.0 TLS设置，支持多个TLS版本")
        } catch (e: Exception) {
            Log.e(TAG, "为Android 7.0配置TLS失败: ${e.message}", e)
        }
    }
