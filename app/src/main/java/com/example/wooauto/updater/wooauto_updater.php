...
class AndroidAppUpdater {
    ...
    public function api_check_update($request) {
        ...
        // 获取客户端传递的当前版本（如果有的话）
        $client_version = $request->get_param('client_version');
        
        // 判断是否有更新：文件存在 AND 版本号不同
        $has_update = false;
        if (!empty($apk_file) && file_exists($apk_file)) {
            if (!empty($client_version)) {
                // 如果客户端提供了版本号，进行版本比较
                $has_update = version_compare($current_version, $client_version, '>');
            } else {
                // 如果没有客户端版本，只要服务器版本不是默认的1.0.0就认为有更新
                $has_update = ($current_version !== '1.0.0');
            }
        }
