...
// 读取GitHub身份验证信息（从local.properties或环境变量中）
...
// 尝试从不同位置获取GitHub认证令牌，默认为空字符串（公共仓库）
val githubAuthToken = localProperties.getProperty("github.auth.token") ?: System.getenv("GITHUB_AUTH_TOKEN") ?: ""
...
android {
    ...
    defaultConfig {
        ...
        // 添加GitHub认证信息（Base64编码的用户名:令牌）
        buildConfigField("String", "GITHUB_AUTH_TOKEN", "\"${githubAuthToken}\"")
    }
