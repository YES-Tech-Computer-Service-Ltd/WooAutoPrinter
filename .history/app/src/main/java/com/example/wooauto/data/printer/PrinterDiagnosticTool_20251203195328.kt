...
object PrinterDiagnosticTool {
    ...
    fun getSocket(connection: DeviceConnection?): BluetoothSocket? {
        ...
        // 策略1：优先尝试直接从 BluetoothConnection 获取（复刻旧代码逻辑，成功率最高）
        try {
            if (connection is BluetoothConnection) {
                val field = BluetoothConnection::class.java.getDeclaredField("socket")
                field.isAccessible = true
                val socket = field.get(connection) as? BluetoothSocket
                if (socket != null) return socket
            }
        } catch (e: Exception) {
            // 忽略异常，继续尝试通用策略
            // Log.w(TAG, "直接反射失败，尝试通用策略: ${e.message}")
        }

        // 策略2：通用循环查找（兼容性更好，应对混淆或类结构变化）
        try {
    ...
    suspend fun runFullDiagnostics(
        ...
        try {
            ...
            // 10 04 01
            sb.append("\n>> 发送指令: 10 04 01 (查询状态)\n")
            // 注意：为了保留完整的日志细节，这里不复用 sendAndReceiveRaw，而是展开写
            // 这样可以记录"无响应"的具体情况，与旧代码体验一致
            try {
                // 清理
                val available = inputStream.available()
                if (available > 0) {
