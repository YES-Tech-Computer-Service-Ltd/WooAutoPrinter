...
fun OrderDetailDialog(
    ...
    OrderDetailDialogContent(
        ...
        printStatusColor = printStatusColor,
        ...
        onStatusOptionsChange = { showStatusOptions = it },
        onTemplateOptionsChange = { showTemplateOptions = it },
        onNavigateToLicenseSettings = { viewModel.navigateToLicenseSettings() }
    )
}
...
fun OrderDetailDialogContent(
    ...
    onStatusChange: (Long, String) -> Unit,
    onMarkAsPrinted: (Long) -> Unit,
    onStatusOptionsChange: (Boolean) -> Unit,
    onTemplateOptionsChange: (Boolean) -> Unit,
        ...
        Card(
            ...
            Column(
                        ...
                        Row(
                            ...
                            Box(
                                modifier = Modifier
                                    ...
                                    .clickable { onStatusOptionsChange(true) }
                            ) {
                                Text(
                                    text = statusText,
                                    style = MaterialTheme.typography.bodyMedium,
                ...
                Row(
                    ...
                    // 主操作：打印
                    Button(
                        onClick = { onTemplateOptionsChange(true) },
                        modifier = Modifier.weight(1f),
                    ...
                    // 次操作：更改订单状态
                    OutlinedButton(
                        onClick = { onStatusOptionsChange(true) },
                        modifier = Modifier.weight(1f),
                        enabled = licenseInfo?.status == LicenseStatus.VALID || licenseInfo?.status == LicenseStatus.TRIAL
                    ) {
                        Icon(
    ...
    if (showStatusOptions) {
        StatusChangeDialog(
            ...
            onDismiss = { onStatusOptionsChange(false) },
            onStatusSelected = { newStatus ->
                onStatusChange(displayOrder.id, newStatus)
                onStatusOptionsChange(false)
            }
        )
    }
    ...
    if (showTemplateOptions) {
        TemplateSelectorDialog(
            onDismiss = { onTemplateOptionsChange(false) },
