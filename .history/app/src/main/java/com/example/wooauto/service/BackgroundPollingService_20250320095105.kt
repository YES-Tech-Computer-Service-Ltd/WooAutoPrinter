        ...
        orders.forEach { order ->
            if (!processedOrderIds.contains(order.id)) {
                ...
                if (isPrintedInDb && !order.isPrinted) {
                    Log.d(TAG, "数据库中订单 #${order.number} 已标记为已打印，但内存中未标记，以数据库为准")
                }
                ...
                // 检查是否需要自动打印，使用数据库中的打印状态
                val shouldPrint = !isPrintedInDb && order.status == "processing"
                Log.d(TAG, "是否需要打印: $shouldPrint (isPrinted=${isPrintedInDb}, status=${order.status})")
