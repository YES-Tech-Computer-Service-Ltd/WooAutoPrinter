...
data class OrderResponse(
    ...
    val deliveryFee: String?
        get() = feeLines?.find { it.name.equals("Shipping fee", ignoreCase = true) }?.total
            ?: feeLines?.find { it.name.equals("shipping fee", ignoreCase = true) }?.total
            ?: feeLines?.find { it.name.equals("SHIPPING FEE", ignoreCase = true) }?.total
            ?: feeLines?.find { it.name.contains("delivery", ignoreCase = true) }?.total
            ?: feeLines?.find { it.name.contains("配送费", ignoreCase = true) }?.total
            ?: feeLines?.find { it.name.contains("外卖费", ignoreCase = true) }?.total
            ?: feeLines?.find { it.name.contains("shipping", ignoreCase = true) }?.total
            ?: feeLines?.find { it.name.contains("运费", ignoreCase = true) }?.total
            ?: feeLines?.find { it.name.contains("freight", ignoreCase = true) }?.total
            ?: feeLines?.find { it.name.contains("transport", ignoreCase = true) }?.total
            ?: feeLines?.find { it.name.contains("配送", ignoreCase = true) }?.total
            ?: feeLines?.find { it.name.contains("运输", ignoreCase = true) }?.total
            ?: feeLines?.find { it.name.contains("送餐", ignoreCase = true) }?.total
            ?: feeLines?.find { it.name.contains("delivery charge", ignoreCase = true) }?.total
