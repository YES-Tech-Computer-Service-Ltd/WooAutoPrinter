...
class ProductRepositoryImpl @Inject constructor(
    ...
    override suspend fun refreshProducts(categoryId: Long?): Result<List<Product>> {
        return try {
            Log.d("ProductRepositoryImpl", "开始刷新产品数据，分类ID: $categoryId")
            ...
            // 注意：WooCommerceApi.getProducts不接受params参数
            val response = api.getProducts(1, 100)
            Log.d("ProductRepositoryImpl", "成功获取产品列表，共 ${response.size} 个产品")
            
            // 如果指定了分类ID，在内存中过滤产品
            val filteredResponse = if (categoryId != null) {
                Log.d("ProductRepositoryImpl", "按分类ID过滤产品: $categoryId")
                response.filter { product ->
                    product.categories?.any { it.id == categoryId } ?: false
                }
            } else {
                response
            }
            Log.d("ProductRepositoryImpl", "过滤后的产品数量: ${filteredResponse.size}")
            
            val entities = ProductMapper.mapResponsesListToEntitiesList(filteredResponse)
            ...
            Log.e("ProductRepositoryImpl", "刷新产品数据时出错: ${e.message}", e)
            // 检查是否是认证错误 (401)
            if (e.message?.contains("401") == true) {
                Log.e("ProductRepositoryImpl", "API认证错误 (401)，请检查consumer key和secret是否正确")
                return Result.failure(Exception("API认证错误，请在设置中检查您的API密钥和密钥"))
            }
            Result.failure(e)
        }
    }
    ...
    override suspend fun updateProduct(product: Product): Result<Product> = withContext(Dispatchers.IO) {
        try {
            ...
            val updatedProduct = ProductMapper.mapResponseToDomain(response)
            // 再次更新本地数据库以确保同步
    ...
    override suspend fun refreshProducts(): Boolean = withContext(Dispatchers.IO) {
        ...
        try {
            val products = apiService.getProducts(1, 100).map { 
                ProductMapper.mapResponseToDomain(it) 
            }
    ...
    override suspend fun clearCache() {
        ...
        _productsByCategoryFlow.value = emptyMap()
        _searchResultsFlow.value = emptyMap()
    }
