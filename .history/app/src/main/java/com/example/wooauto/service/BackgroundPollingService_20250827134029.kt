        ...
        serviceScope.launch {
            try {
                ...
                val defaultConfig = try {
                    settingRepository.getDefaultPrinterConfig()
                } catch (e: Exception) {
                ...
                val currentStatus = try {
                    printerManager.getPrinterStatus(defaultConfig)
                } catch (e: Exception) {
                ...
                Log.d(TAG, "尝试自动连接默认打印机: ${defaultConfig.name} (${defaultConfig.address})")
                val connected = try {
                    withTimeoutOrNull(15000) {
                        printerManager.connect(defaultConfig)
                    } ?: false
                } catch (e: Exception) {
