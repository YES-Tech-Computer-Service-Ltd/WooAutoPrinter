...
class OrderRepositoryImpl @Inject constructor(
    ...
    private suspend fun getApi(config: WooCommerceConfig? = null): WooCommerceApi {
        val actualConfig = config ?: settingsRepository.getWooCommerceConfig().first()
    ...
    override suspend fun updateOrderStatus(orderId: Long, newStatus: String): Result<Order> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val updatedOrder = OrderMapper.mapEntityToDomain(OrderMapper.mapResponseToEntity(response))
    ...
    override suspend fun refreshOrders(status: String?, afterDate: Date?): Result<List<Order>> = withContext(Dispatchers.IO) {
        ...
        try {
            val api = getApi()
            val response = api.getOrders(status, page = 1)
            val orders = response.map { OrderMapper.mapEntityToDomain(OrderMapper.mapResponseToEntity(it)) }
