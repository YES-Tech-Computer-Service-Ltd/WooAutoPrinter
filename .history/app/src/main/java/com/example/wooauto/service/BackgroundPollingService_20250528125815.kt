...
class BackgroundPollingService : Service() {
    ...
    override fun onCreate() {
        ...
        try {
            ...
            lifecycleScope.launch {
                try {
                    val savedInterval = dataStore.data.first()[POLLING_INTERVAL_KEY] ?: DEFAULT_POLLING_INTERVAL
    ...
    private suspend fun pollOrders() {
        try {
            ...
            // 获取待处理订单
            val pollResult = ordersRepository.pollOrders(latestPolledDate)
            
            if (pollResult.isSuccess) {
                val orders = pollResult.getOrNull() ?: emptyList()
