...
class BackgroundPollingService : Service() {
    ...
    private fun initNetworkListener() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            initModernNetworkListener()
        } else {
            // Android 7.0以下使用传统方式
            initLegacyNetworkListener()
        }
    }

    /**
     * 初始化现代网络监听器（Android 7.0+）
     */
    private fun initModernNetworkListener() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            Log.d(TAG, "现代网络监听器已注册（Android 7.0+）")
        }
    }

    /**
     * 初始化传统网络监听器（Android 7.0以下）
     */
    private fun initLegacyNetworkListener() {
        try {
            Log.d(TAG, "初始化传统网络监听器（Android 6.0及以下）")
            ...
            val legacyNetworkReceiver = object : android.content.BroadcastReceiver() {
                override fun onReceive(context: Context?, intent: Intent?) {
                    when (intent?.action) {
                        ConnectivityManager.CONNECTIVITY_ACTION -> {
                            checkNetworkStatusLegacy()
                        }
                        "android.net.wifi.WIFI_STATE_CHANGED" -> {
                            Log.d(TAG, "WiFi状态变化检测")
                            // 延迟检查，因为WiFi状态变化可能需要时间稳定
                            serviceScope.launch {
                                delay(2000)
                                checkNetworkStatusLegacy()
                            }
            ...
            registerReceiver(legacyNetworkReceiver, filter)
            Log.d(TAG, "传统网络监听器注册成功")
            
            // 初始检查网络状态
            checkNetworkStatusLegacy()
            
        } catch (e: Exception) {
            Log.e(TAG, "传统网络监听器初始化失败: ${e.message}")
            // 即使失败也设置默认状态
            isNetworkAvailable = true
        }
    }
     ...
     * 传统方式检查网络状态
     */
    private fun checkNetworkStatusLegacy() {
        try {
            ...
            isNetworkAvailable = activeNetworkInfo?.isConnected == true
            
            Log.d(TAG, "传统网络状态检查: $wasAvailable -> $isNetworkAvailable")
            
            // 如果从离线恢复到在线，触发网络恢复处理
            if (!wasAvailable && isNetworkAvailable && isPollingActive && initialPollingComplete) {
                Log.d(TAG, "传统方式检测到网络恢复，触发恢复轮询")
                serviceScope.launch {
                    try {
                        delay(2000) // 给网络更多时间稳定
                        val result = orderRepository.refreshProcessingOrdersForPolling(latestPolledDate)
                        if (result.isSuccess) {
                            val orders = result.getOrDefault(emptyList())
                            if (orders.isNotEmpty()) {
                                Log.d(TAG, "传统网络恢复轮询成功，获取了 ${orders.size} 个订单")
                                processNewOrders(orders)
                            }
                        }
                    } catch (e: Exception) {
                        Log.e(TAG, "传统网络恢复轮询失败: ${e.message}")
                    }
                }
            }
        } catch (e: Exception) {
            Log.e(TAG, "传统网络状态检查失败: ${e.message}")
            // 出错时假设网络可用，避免影响正常轮询
            isNetworkAvailable = true
        }
    }
