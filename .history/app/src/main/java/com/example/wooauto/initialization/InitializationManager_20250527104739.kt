...
class InitializationManager @Inject constructor(
    ...
    private suspend fun loadBasicConfiguration() {
        ...
        try {
            ...
            withTimeoutOrNull(5000) {
                // 读取轮询间隔
                val pollingInterval = settingsRepository.getPollingInterval()
                Log.d(TAG, "【初始化管理器】预加载轮询间隔: ${pollingInterval}秒")
    ...
    private suspend fun shouldStartBackgroundService(): Boolean {
        return try {
            ...
            val hasSiteUrl = settingsRepository.getSiteUrl().isNotBlank()
            val hasConsumerKey = settingsRepository.getConsumerKey().isNotBlank()
            val hasConsumerSecret = settingsRepository.getConsumerSecret().isNotBlank()
