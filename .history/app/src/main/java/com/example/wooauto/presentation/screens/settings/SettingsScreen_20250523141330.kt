    ...
    Scaffold(
        ...
        if (showPrintTemplatesDialog) {
            Dialog(
                ...
                // Printer Settings Dialog        if (showPrinterSettingsDialog) {            Dialog(                onDismissRequest = { showPrinterSettingsDialog = false },                properties = DialogProperties(usePlatformDefaultWidth = false)            ) {                Card(                    modifier = Modifier                        .fillMaxWidth(0.95f)                        .fillMaxHeight(0.9f),                    shape = RoundedCornerShape(16.dp),                    elevation = CardDefaults.cardElevation(defaultElevation = 8.dp)                ) {                    PrinterSettingsDialogContent(                        viewModel = viewModel,                        onClose = { showPrinterSettingsDialog = false },                        navController = navController                    )                }            }        }        // Store Settings Dialog        if (showStoreSettingsDialog) {            Dialog(                onDismissRequest = { showStoreSettingsDialog = false },                properties = DialogProperties(                    usePlatformDefaultWidth = false,                    dismissOnBackPress = true,                    dismissOnClickOutside = true                )            ) {                StoreSettingsDialogContent(                    viewModel = viewModel,                    onClose = { showStoreSettingsDialog = false }                )            }        }    }}
