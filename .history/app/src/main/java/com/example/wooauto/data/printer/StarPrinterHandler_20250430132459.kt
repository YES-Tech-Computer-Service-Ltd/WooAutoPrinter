...
import com.starmicronics.stario10.starxpand.StarXpandCommand
import com.starmicronics.stario10.starxpand.DocumentBuilder
import com.starmicronics.stario10.starxpand.PrinterBuilder
import com.starmicronics.stario10.starxpand.MagnificationParameter
import com.starmicronics.stario10.starxpand.printer.*
...
class StarPrinterHandler(private val context: Context) {
    ...
    suspend fun printTSP100IIIRasterTest(): Boolean {
        ...
        try {
            ...
            Log.d(TAG, "$logPrefix 开始打印光栅模式测试页...")
            Log.d(TAG, "$logPrefix 打印机型号: $lastConnectedModel")
            ...
            Log.d(TAG, "$logPrefix 发送光栅初始化命令: $initHex")
            currentPrinter?.printAsync(RASTER_MODE_INIT)?.await()
            delay(300)
            ...
            Log.d(TAG, "$logPrefix 发送光栅配置命令: $startHex")
            currentPrinter?.printAsync(RASTER_MODE_START)?.await()
            delay(300)
            
            // 4. 光栅模式文本测试
            val testItems = arrayOf(
                Pair(RASTER_MODE_ALIGN_CENTER, "*** 光栅模式测试 ***\n\n"),
                Pair(RASTER_MODE_ALIGN_LEFT, "1. 左对齐测试\n"),
                Pair(RASTER_MODE_ALIGN_CENTER, "2. 居中对齐测试\n"),
                Pair(RASTER_MODE_ALIGN_RIGHT, "3. 右对齐测试\n"),
                Pair(RASTER_MODE_ALIGN_LEFT + RASTER_MODE_BOLD_ON, "4. 加粗测试"),
                Pair(RASTER_MODE_BOLD_OFF + "\n\n", ""),
                Pair(RASTER_MODE_ALIGN_CENTER, "----- 分隔线 -----\n\n"),
                Pair(RASTER_MODE_ALIGN_LEFT, "打印机型号: $lastConnectedModel\n"),
                Pair(RASTER_MODE_ALIGN_LEFT, "时间: ${java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(java.util.Date())}\n\n"),
                Pair(RASTER_MODE_ALIGN_CENTER, "测试页结束\n\n\n")
            )
            
            // 发送测试项目
            for ((index, item) in testItems.withIndex()) {
                val (command, text) = item
                val combined = command + text
                
                Log.d(TAG, "$logPrefix 测试项[$index]: ${text.replace("\n", "\\n")}")
                
                try {
                    currentPrinter?.printAsync(combined)?.await()
                    delay(100)
                    Log.d(TAG, "$logPrefix 测试项[$index]打印成功")
                } catch (e: Exception) {
                    Log.e(TAG, "$logPrefix 测试项[$index]打印失败: ${e.message}")
                }
            }
            
            // 5. 切纸
            Log.d(TAG, "$logPrefix 发送切纸命令")
            currentPrinter?.printAsync(RASTER_MODE_CUT)?.await()
            delay(500)
            
            // 6. 检查状态
            val status = currentPrinter?.getStatusAsync()?.await()
            Log.d(TAG, "$logPrefix 测试完成, 状态: $status")
