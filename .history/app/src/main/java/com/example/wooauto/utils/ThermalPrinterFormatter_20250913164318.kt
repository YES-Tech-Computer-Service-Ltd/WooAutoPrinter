...
class ThermalPrinterFormatter {
    companion object {
        ...
        /**
         * 当放大时，为连续中文之间插入半角空格，提升可读性（与旧实现一致但不强制加粗）
         */
        private fun applyChineseSpacingIfNeeded(text: String, large: Boolean): String {
            if (!large) return text
            if (!hasChinese(text)) return text
            val sb = StringBuilder()
            for (i in text.indices) {
                sb.append(text[i])
                if (i < text.length - 1 && text[i].code in 0x4E00..0x9FFF && text[i + 1].code in 0x4E00..0x9FFF) {
                    sb.append(' ')
                }
            }
            return sb.toString()
        ...
        /**
         * 通用样式包装：根据 bold / large 添加<b> 与 <h><w> 标签，并提供对齐输出。
         * 不直接写入流，只返回格式化字符串，供最终拼接。
         */
        fun formatStyledLine(
            text: String,
            paperWidth: Int,
            bold: Boolean = false,
            large: Boolean = false,
            ...
            val spaced = applyChineseSpacingIfNeeded(text, large)
            var body = spaced
            if (bold) body = "<b>$body</b>"
            if (large) body = "<h><w>$body</w></h>"
            return "[${'$'}alignment]$body\n"
        }
