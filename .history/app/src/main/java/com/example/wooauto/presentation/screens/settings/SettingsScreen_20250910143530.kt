...
import com.example.wooauto.presentation.components.WooSecondarySidebar
    ...
    Scaffold(
        ...
        Column(
            ...
            Column(
                ...
                // 设置页面的内容区域：二级侧边栏 + 右侧内容
                val subEntries = remember { AppNavConfig.subEntriesForRoute(com.example.wooauto.navigation.NavigationItem.Settings.route) }
                val spec = remember { AppNavConfig.secondarySidebarSpecForRoute(com.example.wooauto.navigation.NavigationItem.Settings.route) }
                val screenWidth = LocalConfiguration.current.screenWidthDp
                val midWidthDp = ((screenWidth * spec.fraction).dp).coerceIn(spec.minDp.dp, spec.maxDp.dp)
                var selectedSub by rememberSaveable {
                    val route = navController.currentBackStackEntry?.arguments?.getString("section")
                    mutableStateOf(route ?: "general")
                }
