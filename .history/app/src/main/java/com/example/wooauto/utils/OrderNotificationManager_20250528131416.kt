...
class OrderNotificationManager @Inject constructor(
    ...
    init {
        try {
            ...
            // 预加载已有订单ID
            preloadExistingOrderIds()
            
            // 延迟一定时间后再开始处理通知，避免启动时的旧订单触发通知
            ...
            Log.e(TAG, "初始化OrderNotificationManager失败: ${e.message}", e)
        }
    }
    ...
    private fun preloadExistingOrderIds() {
        try {
    ...
    private fun registerNewOrderReceiver() {
        try {
            ...
            val intentFilter = IntentFilter("com.example.wooauto.NEW_ORDER")
            ...
            try {
                ...
                // 降级使用传统方式
                context.registerReceiver(newOrderReceiver, intentFilter)
                Log.d(TAG, "降级使用传统方式注册广播接收器")
            }
        } catch (e2: Exception) {
            Log.e(TAG, "所有注册方法均失败: ${e2.message}", e2)
