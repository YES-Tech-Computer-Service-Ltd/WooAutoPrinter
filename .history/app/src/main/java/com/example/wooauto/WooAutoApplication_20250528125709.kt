...
class WooAutoApplication : MultiDexApplication(), Configuration.Provider {
    ...
    override fun onCreate() {
        ...
        try {
            ...
            Thread.setDefaultUncaughtExceptionHandler { thread, exception ->
                Log.e("WooAutoApplication", "未捕获异常 in thread ${thread.name}", exception)
            }
     ...
     * 执行遗留的初始化任务
     */
    private fun performLegacyInitialization() {
        isInitialized = true
        ...
        // 初始化元数据处理系统
        initializeMetadataProcessingSystem()
    ...
    private fun initializeOrderNotificationManager() {
        try {
            // orderNotificationManager会通过Hilt自动注入
        } catch (e: Exception) {
     ...
     * 初始化元数据处理系统（懒加载方式）
     */
    private fun initializeMetadataProcessingSystem() {
        try {
     ...
     * 初始化证书管理器并执行首次验证
     */
    private fun initializeLicenseManagement() {
        try {
    ...
    private fun initializeApplication() {
        ...
        applicationScope.launch {
            try {
                ...
                Log.e("WooAutoApplication", "应用初始化失败", e)
            }
        }
    }
    ...
    private suspend fun completeLegacyTasks() {
        try {
            // 这里可以添加任何需要在应用启动完成后执行的任务
        } catch (e: Exception) {
            Log.e("WooAutoApplication", "遗留任务完成失败: ${e.message}", e)
        }
    }
} 