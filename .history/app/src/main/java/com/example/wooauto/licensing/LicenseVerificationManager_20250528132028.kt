...
class LicenseVerificationManager @Inject constructor(
     ...
     * 启动时验证许可证
     */
    fun verifyLicenseOnStart(context: Context, coroutineScope: CoroutineScope) {
        licenseManager.verifyLicense(context, coroutineScope)
    }

    /**
     * 强制服务器验证
     */
    fun forceServerValidation(context: Context, coroutineScope: CoroutineScope, callback: (Boolean) -> Unit) {
        coroutineScope.launch {
            val result = licenseManager.forceRevalidateAndSync(context)
            callback(result)
        }
    }
    ...
    companion object {
        ...
        fun staticVerifyLicenseOnStart(
            ...
            getInstance().verifyLicenseOnStart(context, coroutineScope)
        }
        ...
        fun staticForceServerValidation(
            ...
            getInstance().forceServerValidation(context, coroutineScope) { isValid ->
                if (isValid) {
                    onSuccess()
                } else {
                    onInvalid()
