...
class NetworkModule {
    ...
    companion object {
        ...
        fun provideOkHttpClient(authInterceptor: WooCommerceAuthInterceptor): OkHttpClient {
            ...
            Log.d("NetworkModule", "配置OkHttpClient，增加超时设置和重试机制以提高稳定性")

            return OkHttpClient.Builder()
                .addInterceptor(loggingInterceptor)
                .addInterceptor(authInterceptor)
                .connectTimeout(60, TimeUnit.SECONDS)  // 增加到60秒
                .readTimeout(60, TimeUnit.SECONDS)     // 增加到60秒
                .writeTimeout(60, TimeUnit.SECONDS)    // 增加到60秒
                .retryOnConnectionFailure(true)        // 启用连接失败重试
                .protocols(listOf(okhttp3.Protocol.HTTP_1_1)) // 强制使用HTTP/1.1，避免HTTP/2的一些问题
                .build()
        }
