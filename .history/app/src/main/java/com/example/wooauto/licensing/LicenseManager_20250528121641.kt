...
class LicenseManager @Inject constructor() {
    ...
    private suspend fun validateLicenseInBackground(licenseKey: String, deviceId: String): Boolean {
        ...
        return try {
            ...
            val result = withTimeoutOrNull(10000) { // 增加到10秒超时
            ...
            try {
                ...
                if (isLicensedLocally && licenseKey.isNotEmpty()) {
                    ...
                    if (licenseValid) {
                        ...
                        // 网络验证失败，检查本地许可证数据是否仍然有效
                        val localEndDate = LicenseDataStore.getLicenseEndDate(context).first()
                        val isLocallyValid = if (localEndDate.isNotEmpty()) {
                            try {
                                val endDate = LicenseDataStore.parseDateString(localEndDate)
                                val isValid = endDate.after(java.util.Date())
                                Log.d("LicenseManager", "📅 [${System.currentTimeMillis()}] 本地许可证检查: endDate=$localEndDate, isValid=$isValid")
                                isValid
                            } catch (e: Exception) {
                                Log.e("LicenseManager", "❌ [${System.currentTimeMillis()}] 本地日期解析失败: ${e.message}")
                                false
                            }
                        } else false
                        ...
                        if (isLocallyValid) {
                            // 本地许可证仍然有效，使用本地数据
                            Log.d("LicenseManager", "🏠 [${System.currentTimeMillis()}] 网络验证失败但本地许可证有效，使用本地数据")
                            updateStatus(LicenseStatus.VALID, message = "网络验证失败，使用本地有效许可证")
                            val totalDuration = System.currentTimeMillis() - totalStartTime
                            Log.d("LicenseManager", "✅ [${System.currentTimeMillis()}] 本地许可证有效，总耗时: ${totalDuration}ms")
                            return@withContext true
                        } else {
                            Log.w("LicenseManager", "⚠️ [${System.currentTimeMillis()}] 网络验证失败且本地许可证无效，检查试用期")
                        }
                    }
                }
                ...
                val trialDays = if (trialValid) {
                    try {
                        ...
                        // 如果试用期天数为0但验证为true，说明可能存在配置问题，给予合理的fallback
                        if (days <= 0 && trialValid) {
                            Log.w("LicenseManager", "⚠️ 试用期验证为true但天数为0，给予3天fallback期")
                            3  // 给3天的fallback期
                        } else {
                            days
                        }
                    ...
                    Log.d("LicenseManager", "📅 [${System.currentTimeMillis()}] 试用期无效，天数设为0")
                    0
                }
