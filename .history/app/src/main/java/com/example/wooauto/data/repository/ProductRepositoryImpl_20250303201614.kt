...
class ProductRepositoryImpl @Inject constructor(
    ...
    override suspend fun refreshProducts(categoryId: Long?): Result<List<Product>> = withContext(Dispatchers.IO) {
        try {
            ...
            try {
                // 根据是否有分类ID来调用不同的API方法
                val dtoList = if (categoryId != null) {
                    // 这里需要自定义实现按分类获取产品的逻辑
                    // 由于WooCommerceApi接口没有提供按分类获取产品的方法
                    // 我们可以先获取所有产品，然后在内存中过滤
                    val allProducts = api.getProducts(1, 100)
                    // 假设ProductDto有一个categories字段，包含分类ID列表
                    allProducts.filter { dto -> 
                        dto.categories.any { it.id == categoryId }
                    }
                } else {
