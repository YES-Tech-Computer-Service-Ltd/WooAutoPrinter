...
class DefaultOrderPrintTemplate @Inject constructor(
    ...
    override fun generateOrderPrintContent(order: Order, config: PrinterConfig): String {
        ...
        return when (templateType) {
            TemplateType.FULL_DETAILS -> generateFullDetailsTemplate(order, config, templateType)
            TemplateType.DELIVERY -> generateDeliveryTemplate(order, config, templateType)
            TemplateType.KITCHEN -> generateKitchenTemplate(order, config, templateType)
        }
    }
     ...
     * 生成完整订单详情模板 - 包含所有信息
     */
    private fun generateFullDetailsTemplate(order: Order, config: PrinterConfig, templateType: TemplateType): String {
        // 获取模板配置
        val templateConfig = runBlocking { 
            templateConfigRepository.getOrCreateConfig("full_details", templateType)
        }
        
        Log.d(TAG, "获取到模板配置: ${templateConfig.templateId}, 显示店铺信息: ${templateConfig.showStoreInfo}")
        ...
        // 客户信息 - 使用模板配置
        if (templateConfig.showCustomerInfo && (order.customerName.isNotEmpty() || order.contactInfo.isNotEmpty() || order.billingInfo.isNotEmpty())) {
            sb.append(formatter.formatDivider(paperWidth))
        ...
        // 配送信息 - 使用模板配置
        if (templateConfig.showDeliveryInfo && order.woofoodInfo != null) {
            sb.append(formatter.formatDivider(paperWidth))
        ...
        if (templateConfig.showItemDetails) {
            ...
            if (templateConfig.showItemPrices) {
                ...
                sb.append(formatter.formatLeftText(formatter.formatBold("Item"), paperWidth))
            }
            ...
            order.items.forEach { item ->
                ...
                if (templateConfig.showItemPrices) {
                    ...
                    sb.append(formatter.formatLeftText("${item.quantity} x $name", paperWidth))
                }
        ...
        // 支付信息 - 使用模板配置
        if (templateConfig.showPaymentInfo) {
            sb.append(formatter.formatLeftRightText("payment method:", order.paymentMethod, paperWidth))
        }
        
        // 订单备注 - 使用模板配置
        if (templateConfig.showOrderNotes && order.notes.isNotEmpty()) {
            sb.append(formatter.formatDivider(paperWidth))
     ...
     */
    private fun generateDeliveryTemplate(order: Order, config: PrinterConfig, templateType: TemplateType): String {
        // 获取模板配置
        val templateConfig = runBlocking { 
            templateConfigRepository.getOrCreateConfig("delivery", templateType)
        }
        
        Log.d(TAG, "获取到配送模板配置: ${templateConfig.templateId}")
        ...
        // 客户和配送信息 (配送单特别突出这部分) - 使用模板配置
        if (templateConfig.showCustomerInfo || templateConfig.showDeliveryInfo) {
            sb.append(formatter.formatDivider(paperWidth))
            sb.append(formatter.formatLeftText(formatter.formatBold("DELIVERY INFORMATION"), paperWidth))
            
            // 客户名称
            if (templateConfig.showCustomerInfo && order.customerName.isNotEmpty()) {
                sb.append(formatter.formatLabelValue("Customer", order.customerName, paperWidth))
            }
            
            // 联系方式 (配送单必须包含)
            if (templateConfig.showCustomerInfo && order.contactInfo.isNotEmpty()) {
                sb.append(formatter.formatLabelValue("Contact", order.contactInfo, paperWidth))
            }
            
            // 配送地址 (配送单重点突出)
            if (templateConfig.showDeliveryInfo) {
        ...
        if (templateConfig.showItemDetails) {
            ...
            order.items.forEach { item ->
                if (templateConfig.showItemPrices) {
                    val price = formatPrice(item.price.toDouble())
                    sb.append(formatter.formatItemPriceLine(item.name, item.quantity.toInt(), price, paperWidth))
                } else {
     ...
     */
    private fun generateKitchenTemplate(order: Order, config: PrinterConfig, templateType: TemplateType): String {
        // 获取模板配置
        val templateConfig = runBlocking { 
            templateConfigRepository.getOrCreateConfig("kitchen", templateType)
        }
        
        Log.d(TAG, "获取到厨房模板配置: ${templateConfig.templateId}")
