...
class StarPrinterHandler(private val context: Context) {
    ...
    suspend fun printText(content: String): Boolean {
        ...
        try {
            ...
            // 检查是否是TSP100系列
            if (isTSP100Series) {
                Log.d(TAG, "$logPrefix 使用TSP100光栅模式打印")
                return printWithRasterMode(content)
            }
    ...
    suspend fun printWithRasterMode(content: String): Boolean {
        ...
        try {
            ...
            val rasterCommands = byteArrayOf(
                0x1B, 0x1D, 0x73, 0x00, 0x30, // ESC GS s 0 0 - 设置光栅打印质量
                0x1B, 0x1D, 0x4C, 0x00, 0x00, // ESC GS L 0 0 - 设置左边距
                0x1B, 0x1D, 0x57, 0x40, 0x01, // ESC GS W 64 1 - 设置打印区域宽度
                0x1B, 0x1D, 0x41, 0x01        // ESC GS A 1 - 启用光栅缓冲
            )
            currentPrinter?.printAsync(String(rasterCommands))?.await()
            delay(300)
            ...
            for (line in lines) {
                ...
                when {
                    line.startsWith("[C]") -> {
                        formattedContent.append(byteArrayOf(0x1B, 0x1D, 0x61, 0x01))  // 居中
                        formattedContent.append(processRasterLine(line.substring(3)))
                    }
                    line.startsWith("[R]") -> {
                        formattedContent.append(byteArrayOf(0x1B, 0x1D, 0x61, 0x02))  // 右对齐
                        formattedContent.append(processRasterLine(line.substring(3)))
                    }
                    line.startsWith("[L]") -> {
                        formattedContent.append(byteArrayOf(0x1B, 0x1D, 0x61, 0x00))  // 左对齐
                        formattedContent.append(processRasterLine(line.substring(3)))
                    }
                    else -> {
                        formattedContent.append(byteArrayOf(0x1B, 0x1D, 0x61, 0x00))  // 默认左对齐
            ...
            Log.d(TAG, "$logPrefix 打印命令序列(HEX): $hexContent")

            // 分段打印
            val chunkSize = 512  // 减小分段大小
            ...
            for (i in contentBytes.indices step chunkSize) {
                ...
                // 检查状态
                val status = currentPrinter?.getStatusAsync()?.await()
                Log.d(TAG, "$logPrefix 打印进度: ${end}/${contentBytes.size}, 状态: $status")
            }

            // 6. 走纸和切纸
            Log.d(TAG, "$logPrefix 走纸和切纸")
            val endCommands = byteArrayOf(
                0x0A, 0x0A, 0x0A,             // 走纸3行
                0x1B, 0x1D, 0x56, 0x42, 0x00  // ESC GS V B 0 - 切纸
            )
            currentPrinter?.printAsync(String(endCommands))?.await()
            delay(500)
    ...
    private fun processRasterLine(line: String): String {
        ...
        var text = line
        if (text.contains("<b>")) {
            text = text.replace("<b>", String(byteArrayOf(0x1B, 0x1D, 0x45, 0x01)))  // 加粗开启
            text = text.replace("</b>", String(byteArrayOf(0x1B, 0x1D, 0x45, 0x00))) // 加粗关闭
        }
        
        // 处理中文字符
        if (text.any { it.code > 127 }) {
            processedLine.append(byteArrayOf(0x1B, 0x1D, 0x74, 0x01))  // 设置中文编码
        }
