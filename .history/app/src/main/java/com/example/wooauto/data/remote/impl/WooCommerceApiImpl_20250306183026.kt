...
class WooCommerceApiImpl(
    ...
    override suspend fun getOrders(page: Int, perPage: Int, status: String?): List<OrderDto> {
        ...
        while (attempt < maxRetries) {
            ...
            try {
                ...
                if (attempt > 1) {
                    ...
                    Log.d("WooCommerceApiImpl", "【API重试】第 $attempt 次重试，延迟 ${delayMs}ms...")
                    delay(delayMs)
                }
                
                // 发送请求获取订单
                Log.d("WooCommerceApiImpl", "【API请求】尝试获取订单，第 $attempt/$maxRetries 次尝试")
                ...
                lastException = e
                Log.e("WooCommerceApiImpl", "【API错误】HTTP/2流重置错误: ${e.message}，尝试重试...")
                // HTTP/2协议错误，继续下一次重试
                continue
            } catch (e: Exception) {
                lastException = e
                ...
                if (e is java.net.SocketTimeoutException || 
                    ...
                    Log.d("WooCommerceApiImpl", "【API重试】网络错误，将重试: ${e.javaClass.simpleName}")
                    continue
                }
        ...
        // 如果所有尝试都失败，抛出最后一个异常
        Log.e("WooCommerceApiImpl", "【API失败】所有重试均失败，放弃获取订单")
