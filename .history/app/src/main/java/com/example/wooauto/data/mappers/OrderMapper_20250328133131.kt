...
object OrderMapper {
    ...
    fun mapEntityToDomain(entity: OrderEntity): Order {
        ...
        // 打印WooFoodInfo内容进行调试
        println("【数据转换】订单#${entity.number} WooFoodInfo: $woofoodInfo")
        
        // 解析额外费用项
        val feeLines = parseFeeLines(entity)
        
        // 尝试从feeLines中提取配送费，更新到WooFoodInfo中
        if (woofoodInfo?.isDelivery == true && (woofoodInfo.deliveryFee == null || woofoodInfo.deliveryFee == "0.00")) {
            ...
            if (deliveryFeeLine != null) {
                // 创建一个新的WooFoodInfo对象，包含更新的配送费
                val updatedWooFoodInfo = com.example.wooauto.domain.models.WooFoodInfo(
                    orderMethod = woofoodInfo.orderMethod,
                    deliveryTime = woofoodInfo.deliveryTime,
                    deliveryAddress = woofoodInfo.deliveryAddress,
                    deliveryFee = deliveryFeeLine.total,  // 使用从feeLines中找到的配送费
                    tip = woofoodInfo.tip,
                    isDelivery = woofoodInfo.isDelivery
                )
                println("【数据修正】将订单#${entity.number}的配送费从${woofoodInfo.deliveryFee}更新为${deliveryFeeLine.total}")
                return createOrderWithUpdatedWooFoodInfo(entity, orderItems, updatedWooFoodInfo, feeLines)
            }
        }
        
        // 继续使用原来的woofoodInfo创建Order对象
        return createOrderWithUpdatedWooFoodInfo(entity, orderItems, woofoodInfo ?: com.example.wooauto.domain.models.WooFoodInfo(
            orderMethod = "pickup",
            deliveryTime = null,
            ...
            try {
                ...
                for (pattern in deliveryFeeApiPatterns) {
                    ...
                    if (deliveryApiMatch != null && deliveryApiMatch.groupValues.size > 1) {
                        ...
                        if (extracted.isNotEmpty() && extracted != "0" && extracted != "0.00") {
                            ...
                            println("从API记录中提取到配送费: $deliveryFee")
                            break
                        }
                    }
                }
                ...
                if (deliveryFee == null || deliveryFee.isEmpty()) {
                    ...
                    if (deliveryFeeMatch != null && deliveryFeeMatch.groupValues.size > 1) {
                        ...
                        println("从常规模式中提取到配送费: $deliveryFee")
                    }
                    ...
                    if (deliveryFee == null || deliveryFee.isEmpty()) {
                        ...
                        if (directMatch != null && directMatch.groupValues.size > 2) {
                            ...
                            println("从直接格式中提取到配送费: $deliveryFee")
                        }
                    }
                }
            } catch (e: Exception) {
    ...
    private fun parseFeeLines(entity: OrderEntity): List<com.example.wooauto.domain.models.FeeLine> {
        ...
        if (entity.customerNote.isNotEmpty()) {
            ...
            for (pattern in deliveryFeePatterns) {
                val deliveryFeeMatch = pattern.find(entity.customerNote)
                if (deliveryFeeMatch != null && deliveryFeeMatch.groupValues.size > 1) {
                    deliveryFeeFromApi = deliveryFeeMatch.groupValues[1].replace("[¥￥$\\s]".toRegex(), "")
                    println("parseFeeLines - 从备注中提取到配送费: $deliveryFeeFromApi")
                    break
                }
            }
            ...
            for (pattern in tipPatterns) {
                val tipMatch = pattern.find(entity.customerNote)
                if (tipMatch != null && tipMatch.groupValues.size > 1) {
                    tipFromApi = tipMatch.groupValues[1].replace("[¥￥$\\s]".toRegex(), "")
                    println("parseFeeLines - 从备注中提取到小费: $tipFromApi")
                    break
                }
            }
        }
        
        // 如果既找不到API记录，也没有woofoodInfo提供的值，就查找常规格式
        if ((deliveryFeeFromApi == null || deliveryFeeFromApi == "0.00") && 
            (woofoodInfo?.deliveryFee == null || woofoodInfo.deliveryFee == "0.00")) {
            ...
            try {
                ...
                if (deliveryFeeMatch != null && deliveryFeeMatch.groupValues.size > 1) {
                    deliveryFeeFromApi = deliveryFeeMatch.groupValues[1].replace("[¥￥$\\s]".toRegex(), "")
                    println("parseFeeLines - 从备注常规格式提取到配送费: $deliveryFeeFromApi")
                }
            } catch (e: Exception) {
                println("parseFeeLines - 从备注提取配送费出错: ${e.message}")
            }
        }
        
        if ((tipFromApi == null || tipFromApi == "0.00") && 
            (woofoodInfo?.tip == null || woofoodInfo.tip == "0.00")) {
            
            // 尝试从订单备注中直接提取小费
            try {
                val tipPattern = "(?:小费|感谢费|Tip|Gratuity)[：:]*\\s*([¥￥$]?\\s*\\d+(\\.\\d+)?)".toRegex(RegexOption.IGNORE_CASE)
                val tipMatch = tipPattern.find(entity.customerNote)
                
                if (tipMatch != null && tipMatch.groupValues.size > 1) {
                    tipFromApi = tipMatch.groupValues[1].replace("[¥￥$\\s]".toRegex(), "")
                    println("parseFeeLines - 从备注常规格式提取到小费: $tipFromApi")
                }
            } catch (e: Exception) {
                println("parseFeeLines - 从备注提取小费出错: ${e.message}")
            }
        }
        
        // 添加配送费（仅配送订单）
        if (isDelivery) {
            // 优先使用API提取的值，然后是WooFoodInfo中的值
            val fee = when {
                deliveryFeeFromApi != null && deliveryFeeFromApi != "0.00" -> deliveryFeeFromApi
                woofoodInfo?.deliveryFee != null && woofoodInfo.deliveryFee != "0.00" -> woofoodInfo.deliveryFee
                else -> "0.00"
            }
            
            println("parseFeeLines - 使用的配送费: $fee (API: $deliveryFeeFromApi, WooFood: ${woofoodInfo?.deliveryFee})")
            
            // 记录外卖订单但配送费为0的情况
            if (fee == "0.00") {
                println("【调试】parseFeeLines - 发现外卖订单但配送费为0: 订单号#${entity.number}")
            }
            ...
            println("订单#${entity.number} 不是外卖订单，不添加配送费")
        }
        
        // 添加小费（无论订单类型）
        // 优先使用API提取的值，然后是WooFoodInfo中的值
        val tip = when {
            tipFromApi != null && tipFromApi != "0.00" -> tipFromApi
            woofoodInfo?.tip != null && woofoodInfo.tip != "0.00" -> woofoodInfo.tip
            else -> "0.00"
        }
        
        println("parseFeeLines - 使用的小费: $tip (API: $tipFromApi, WooFood: ${woofoodInfo?.tip})")
        ...
        // 显示WooFoodInfo内容，帮助调试
        println("【调试数据】WooFoodInfo: ${woofoodInfo.toString()}")
        println("WooFoodInfo.isDelivery: ${woofoodInfo?.isDelivery}")
        println("WooFoodInfo.deliveryFee: ${woofoodInfo?.deliveryFee}")
        println("WooFoodInfo.tip: ${woofoodInfo?.tip}")
    ...
    private fun createOrderWithUpdatedWooFoodInfo(
        ...
        updatedWooFoodInfo: com.example.wooauto.domain.models.WooFoodInfo,
        ...
        val finalFeeLines = if (hasFeeTip) {
            ...
            val tipFeeLine = com.example.wooauto.domain.models.FeeLine(
                id = feeLines.size + 1L,
                name = "小费",
                total = updatedWooFoodInfo.tip ?: "0.00",
                totalTax = "0.00"
            )
            feeLines + tipFeeLine
        }
        ...
        // 如果是外卖订单，检查是否有配送费
        val isDelivery = updatedWooFoodInfo.isDelivery
        ...
        return Order(
            ...
            woofoodInfo = updatedWooFoodInfo, // 使用更新后的WooFoodInfo
