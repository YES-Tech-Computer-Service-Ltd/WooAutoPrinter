...
class BackgroundPollingService : Service() {
    ...
    private fun startPolling() {
        ...
        pollingJob = serviceScope.launch {
            ...
            Log.d(TAG, "🔄 轮询主循环开始 - isActive: $isActive, isPollingActive: $isPollingActive")
                ...
                while (isActive && isPollingActive) {
                    try {
                        ...
                        if (isValid) {
                            ...
                            Log.w(TAG, "❌ 配置未完成，跳过轮询，等待${effectiveInterval}秒后重试")
                        ...
                        // 其他异常继续轮询，但等待指定间隔
                        Log.w(TAG, "⚠️ 异常后继续轮询，等待${currentPollingInterval}秒")
                        delay(currentPollingInterval * 1000L)
                    }
                }
                
                Log.w(TAG, "⏹️ 轮询主循环结束 - isActive: $isActive, isPollingActive: $isPollingActive")
            } finally {
                isPollingActive = false
                Log.d(TAG, "🔴 轮询任务结束")
            }
