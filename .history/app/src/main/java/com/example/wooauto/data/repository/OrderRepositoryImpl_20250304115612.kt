...
class OrderRepositoryImpl @Inject constructor(
    ...
    override suspend fun refreshOrders(status: String?, afterDate: Date?): Result<List<Order>> = withContext(Dispatchers.IO) {
        Log.d("OrderRepositoryImpl", "【订单调试】开始刷新订单数据，状态: ${status ?: "全部"}, 日期过滤: ${afterDate?.toString() ?: "无"}")
        
        try {
            // 获取API配置并打印关键信息
            val config = settingsRepository.getWooCommerceConfig()
            Log.d("OrderRepositoryImpl", "【订单调试】API配置 - 站点URL: ${config.siteUrl}, " +
                    "配置是否有效: ${config.isValid()}, " +
                    "Consumer Key: ${config.consumerKey.take(5)}..., " +
                    "Secret: ${config.consumerSecret.take(5)}...")
            
            // 检查配置有效性
            if (!config.isValid()) {
                Log.e("OrderRepositoryImpl", "【订单调试】API配置无效，请检查设置")
            ...
            if (status != null) {
                ...
                Log.d("OrderRepositoryImpl", "【订单调试】使用状态过滤: $status")
            }
            
            // 添加日期过滤
            if (afterDate != null) {
                queryParams["after"] = afterDate.toString()
                Log.d("OrderRepositoryImpl", "【订单调试】使用日期过滤: $afterDate")
            }
            
            Log.d("OrderRepositoryImpl", "【订单调试】调用API获取订单列表，参数: $queryParams")
            
            try {
                // 修改此处，正确传递状态参数
                val response = if (status != null) {
                    api.getOrders(1, 100, status)
                } else {
                    api.getOrders(1, 100)
                }
                Log.d("OrderRepositoryImpl", "【订单调试】API调用成功，获取到 ${response.size} 个订单")
                
                // 查看返回的原始数据
                if (response.isNotEmpty()) {
                    val firstOrder = response.first()
                    Log.d("OrderRepositoryImpl", "【订单调试】第一个订单示例 - ID: ${firstOrder.id}, 状态: ${firstOrder.status}, 总金额: ${firstOrder.total}")
                } else {
                    Log.d("OrderRepositoryImpl", "【订单调试】API返回了空的订单列表")
                }
                
                // 转换为领域模型
                val orders = response.map { it.toOrder() }
                Log.d("OrderRepositoryImpl", "【订单调试】成功将 ${orders.size} 个订单转换为领域模型")
                ...
                Log.d("OrderRepositoryImpl", "【订单调试】准备更新数据库，实体数量: ${entities.size}")
                orderDao.deleteAllOrders() // 清除旧数据
                orderDao.insertOrders(entities) // 插入新数据
                Log.d("OrderRepositoryImpl", "【订单调试】数据库更新完成")
                ...
                Log.e("OrderRepositoryImpl", "【订单调试】API调用异常: ${e.javaClass.simpleName} - ${e.message}", e)
                // 检查是否是HTTP异常，如果是，打印状态码
                if (e.message?.contains("HTTP") == true) {
                    Log.e("OrderRepositoryImpl", "【订单调试】可能是HTTP错误，详细信息: ${e.message}")
                }
                throw e
            }
        } catch (e: Exception) {
            ...
            Log.e("OrderRepositoryImpl", "【订单调试】刷新订单数据失败: ${error.code} - ${error.message}", e)
            if (error.code == 401) {
                Log.e("OrderRepositoryImpl", "【订单调试】认证错误 (401)，请检查Consumer Key和Secret是否正确")
            } else if (error.code == 404) {
                Log.e("OrderRepositoryImpl", "【订单调试】API端点未找到 (404)，请检查URL是否正确")
            }
    ...
    private fun updateOrdersByStatus() {
        ...
        Log.d("OrderRepositoryImpl", "【状态调试】更新订单状态分组，所有状态: ${groupedOrders.keys.joinToString()}")
        
        // 记录每个状态的订单数量
        groupedOrders.forEach { (status, orders) ->
            Log.d("OrderRepositoryImpl", "【状态调试】状态 '$status' 有 ${orders.size} 个订单")
            if (orders.isNotEmpty()) {
                val sample = orders.take(2).joinToString(", ") { "#${it.id}" }
                Log.d("OrderRepositoryImpl", "【状态调试】状态 '$status' 订单示例: $sample${if (orders.size > 2) "..." else ""}")
            }
        }
