    ...
    override suspend fun refreshOrders(status: String?, afterDate: Date?): Result<List<Order>> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val apiStatus = if (status != null) {
                // 先尝试直接映射中文状态到英文状态
                val mappedStatus = statusMap[status.trim()] ?: status.trim()
                
                // 再检查映射后的状态是否是有效的API状态
                val finalStatus = if (validApiStatuses.contains(mappedStatus.lowercase())) {
                    mappedStatus.lowercase()
                } else {
                    // 如果不是有效状态，改用"any"
                    Log.w("OrderRepositoryImpl", "【状态刷新】警告：'$mappedStatus' 不是有效的API状态，改用 'any'")
                    "any"
                }
                
                Log.d("OrderRepositoryImpl", "【状态刷新】状态映射: '$status' -> '$mappedStatus' -> '$finalStatus'")
