...
object OrderMapper {
    ...
    fun mapResponseToEntity(response: OrderResponse): OrderEntity {
        ...
        // 1. 检查是否有外卖相关数据
        response.orderMethod?.let { metaData.append("订单方式: $it\n") }
        response.deliveryTime?.let { metaData.append("送达时间: $it\n") }
        response.deliveryDate?.let { metaData.append("送达日期: $it\n") }
        
        // 2. 提取外卖费
        val deliveryFee = response.deliveryFee
        if (deliveryFee != null && deliveryFee != "0.00") {
            metaData.append("外卖费: $deliveryFee\n")
        }
        
        // 3. 提取小费 - 查找fee_lines中的"Show Your Appreciation"或类似项
        var tipAmount: String? = null
        response.feeLines?.forEach { feeLine ->
            if (feeLine.name.contains("Show Your Appreciation", ignoreCase = true) || 
                feeLine.name.contains("Tip", ignoreCase = true) || 
                feeLine.name.contains("小费", ignoreCase = true) ||
                feeLine.name.contains("感谢", ignoreCase = true)) {
        ...
        response.metaData.forEach { meta ->
            if (meta.key.contains("woofood", ignoreCase = true) || 
                ...
                metaData.append("${meta.key}: ${meta.value}\n")
            }
        }
        
        // 将元数据添加到订单备注中
        val customerNote = if (metaData.isNotEmpty()) {
