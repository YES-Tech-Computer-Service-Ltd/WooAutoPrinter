...
class WooCommerceApiImpl(
     ...
     * 构建API URL
     */
    private fun buildApiUrl(endpoint: String, params: Map<String, String?>): okhttp3.HttpUrl {
        val baseUrl = buildBaseUrl()
        // 确保不存在双斜杠
        val cleanEndpoint = if (endpoint.startsWith("/")) endpoint.substring(1) else endpoint
        val fullApiUrl = "$baseUrl$cleanEndpoint"
        Log.d(TAG, "【URL构建】基本URL: $baseUrl, 终端点: $cleanEndpoint")
        Log.d(TAG, "【URL构建】合并URL: $fullApiUrl")
        ...
        params.forEach { (key, value) ->
            if (value != null) {
                Log.d(TAG, "【URL构建】添加参数: $key=$value")
                urlBuilder.addQueryParameter(key, value)
            }
        }
     ...
     * 发送GET请求并返回响应
     */
    private suspend fun makeGetRequest(url: okhttp3.HttpUrl): okhttp3.Response {
        return withContext(Dispatchers.IO) {
            val request = Request.Builder()
                .url(url)
                .get()
     ...
     * 解析订单响应
     */
    private fun parseOrdersResponse(responseBody: okhttp3.ResponseBody?): List<OrderDto> {
        val body = responseBody?.string() ?: return emptyList()
        
        return try {
            // 尝试使用注册的适配器直接解析
            gson.fromJson<List<OrderDto>>(body, orderDtoListType)
        } catch (e: Exception) {
            Log.e(TAG, "标准订单解析失败，尝试手动解析: ${e.message}")
            // 如果标准解析失败，尝试手动解析
            extractOrdersManually(body)
        }
    }
} 