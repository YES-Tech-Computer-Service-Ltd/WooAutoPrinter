...
fun OrdersActivePlaceholderScreen(
    ...
    BoxWithConstraints(modifier = Modifier.fillMaxSize().padding(pad).padding(12.dp)) {
        val spacing = 8.dp
        val dividerWidth = 1.dp
        val sectionWidth = (maxWidth - spacing - dividerWidth - spacing) / 2
        Row(modifier = Modifier.fillMaxSize()) {
        Column(modifier = Modifier.fillMaxHeight().width(sectionWidth)) {
        ...
        Box(modifier = Modifier.fillMaxHeight().width(dividerWidth).background(MaterialTheme.colorScheme.outlineVariant.copy(alpha = 0.35f)))
        Spacer(modifier = Modifier.width(spacing))
        Column(modifier = Modifier.fillMaxHeight().width(sectionWidth)) {
