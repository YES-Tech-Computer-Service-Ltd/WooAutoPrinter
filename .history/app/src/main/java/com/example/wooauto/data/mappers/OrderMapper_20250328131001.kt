...
object OrderMapper {
        ...
        if (isDelivery) {
            try {
                ...
                if (deliveryFee == null || deliveryFee.isEmpty()) {
                    // 外卖费可能出现在元数据中
                    val deliveryFeePattern = "(?:外卖费|配送费|运费|送餐费|Shipping fee|shipping fee|SHIPPING FEE|Delivery fee|delivery charge|运输费)[：:]*\\s*([¥￥$]?\\s*\\d+(\\.\\d+)?)".toRegex(RegexOption.IGNORE_CASE)
    ...
    private fun parseFeeLines(entity: OrderEntity): List<com.example.wooauto.domain.models.FeeLine> {
        ...
        if ((deliveryFeeFromApi == null || deliveryFeeFromApi == "0.00") && 
            ...
            // 尝试从订单备注中直接提取配送费
            try {
                val deliveryFeePattern = "(?:外卖费|配送费|运费|送餐费|Shipping fee|Delivery fee|delivery charge)[：:]*\\s*([¥￥$]?\\s*\\d+(\\.\\d+)?)".toRegex(RegexOption.IGNORE_CASE)
                val deliveryFeeMatch = deliveryFeePattern.find(entity.customerNote)
