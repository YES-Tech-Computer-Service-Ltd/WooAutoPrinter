...
class WooCommerceApiImpl(
            ...
            try {
                return withContext(Dispatchers.IO) {
                    try {
                        ...
                        // 检查是否是Android 7，添加特殊处理
                        val isAndroid7 = try {
                            val androidVersion = android.os.Build.VERSION.SDK_INT
                            androidVersion in 24..25  // API 24-25 = Android 7.0-7.1
                        } catch (e: Exception) {
                            false
                        }
                        ...
                        // 执行网络请求
                        val response = try {
                            // 如果是Android 7的订单请求，尝试更保守的方法
                            if (isAndroid7 && isOrderRequest && currentRetry > 0) {
                                Log.d("WooCommerceApiImpl", "【Android 7】使用保守方法进行订单请求")
                                // 创建一个临时自定义客户端，强制使用更低的TLS版本
                                val tempClient = createLegacyClient()
                            ...
                            if (isOrderRequest) {
                                try {
                                    Log.d("WooCommerceApiImpl", "【SSL错误恢复】SSL握手失败，立即尝试降级TLS")
                                    val tempClient = createLegacyClient()
                                    tempClient.newCall(request).execute()
                                } catch (e2: Exception) {
                                    Log.e("WooCommerceApiImpl", "【SSL降级失败】降级TLS也失败: ${e2.message}", e2)
            ...
            try {
                return withContext(Dispatchers.IO) {
                    try {
                        ...
                        // 执行网络请求
                        val response = try {
                            // 如果是Android 7的订单请求且重试中，使用更保守的客户端
                            if (isAndroid7 && isOrderRequest && currentRetry > 0) {
                                Log.d("WooCommerceApiImpl", "【Android 7】使用保守客户端进行订单POST请求")
                                val tempClient = createLegacyClient()
                                tempClient.newCall(request).execute()
                            } else {
                                client.newCall(request).execute()
                            ...
                            Log.e("WooCommerceApiImpl", "【POST请求】SSL握手失败: ${e.message}", e)
                            // 如果是订单请求，尝试立即使用降级客户端
                            if (isOrderRequest) {
                                try {
                                    Log.d("WooCommerceApiImpl", "【POST请求】SSL握手失败，尝试降级TLS")
                                    val tempClient = createLegacyClient()
                                    tempClient.newCall(request).execute()
                                } catch (e2: Exception) {
                                    Log.e("WooCommerceApiImpl", "【POST请求】降级TLS也失败: ${e2.message}")
                                    throw Exception("SSL安全连接失败: ${e.message}")
                                }
                            } else {
                                throw Exception("SSL安全连接失败: ${e.message}")
                            }
                        } catch (e: Exception) {
                            Log.e("WooCommerceApiImpl", "【POST请求】请求执行失败: ${e.message}", e)
                            throw e
                        }
    ...
    // 创建一个适合Android 7的旧版（Legacy）HTTP客户端
    private fun createLegacyClient(): OkHttpClient {
        Log.d("WooCommerceApiImpl", "创建旧版兼容HTTP客户端，针对Android 7")
        ...
        val legacySslContext = try {
            // 明确使用TLSv1.0，这在Android 7上可能更兼容
            val context = javax.net.ssl.SSLContext.getInstance("TLSv1")
            context.init(null, trustAllCerts, java.security.SecureRandom())
            context
        } catch (e: Exception) {
            Log.e("WooCommerceApiImpl", "创建Legacy SSL上下文失败: ${e.message}", e)
            val fallback = javax.net.ssl.SSLContext.getInstance("TLS")
            fallback.init(null, trustAllCerts, java.security.SecureRandom())
            fallback
        }
        ...
        return OkHttpClient.Builder()
            ...
            .readTimeout(30, TimeUnit.SECONDS)
            .writeTimeout(30, TimeUnit.SECONDS)
            .protocols(listOf(okhttp3.Protocol.HTTP_1_1)) // 强制HTTP/1.1
            .connectionSpecs(listOf(legacySpec, okhttp3.ConnectionSpec.CLEARTEXT))
            .sslSocketFactory(legacySslContext.socketFactory, trustAllCerts[0] as javax.net.ssl.X509TrustManager)
            .hostnameVerifier { _, _ -> true }
            .retryOnConnectionFailure(true)
            .build()
    }
