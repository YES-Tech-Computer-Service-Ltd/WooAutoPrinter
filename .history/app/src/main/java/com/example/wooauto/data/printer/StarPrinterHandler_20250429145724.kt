...
class StarPrinterHandler(private val context: Context) {
    ...
    suspend fun printWithRasterMode(content: String): Boolean {
        ...
        try {
            ...
            // 记录打印内容
            Log.d(TAG, "$logPrefix 原始打印内容: ${content.take(100)}...")
            Log.d(TAG, "$logPrefix 打印内容长度: ${content.length}")
            Log.d(TAG, "$logPrefix 打印机型号: $lastConnectedModel")
            Log.d(TAG, "$logPrefix 打印机类型是否为TSP100系列: $isTSP100Series")
            Log.d(TAG, "$logPrefix 内容行数: ${content.lines().size}")
            
            // 记录内容前10行，帮助诊断问题
            content.lines().take(10).forEachIndexed { index, line ->
            ...
            try {
                ...
                Log.e(TAG, "$logPrefix 异常详情: ${e.stackTraceToString().take(500)}...")
            }
            delay(500)
            ...
            try {
                ...
                Log.d(TAG, "$logPrefix 初始化打印机成功，发送命令HEX: ${DEBUG_RASTER_INIT.toByteArray().joinToString(" ") { String.format("%02X", it.toInt() and 0xFF) }}")
            } catch (e: Exception) {
            ...
            try {
                ...
                Log.d(TAG, "$logPrefix 启动光栅模式成功，发送命令HEX: ${DEBUG_RASTER_START.toByteArray().joinToString(" ") { String.format("%02X", it.toInt() and 0xFF) }}")
            } catch (e: Exception) {
            ...
            for ((index, line) in lines.withIndex()) {
                Log.d(TAG, "$logPrefix 处理第 ${index+1}/${lines.size} 行: ${if(line.length > 30) line.substring(0, 30) + "..." else line}")
                // 处理对齐方式
                when {
                    line.startsWith("[C]") -> {
                        formattedContent.append(RASTER_MODE_ALIGN_CENTER)
                        val processedLine = processRasterLine(line.substring(3))
                        formattedContent.append(processedLine)
                        Log.d(TAG, "$logPrefix 第 ${index+1} 行使用居中对齐，处理后: ${processedLine.take(20)}${if(processedLine.length > 20) "..." else ""}")
                    }
                    line.startsWith("[R]") -> {
                        formattedContent.append(RASTER_MODE_ALIGN_RIGHT)
                        val processedLine = processRasterLine(line.substring(3))
                        formattedContent.append(processedLine)
                        Log.d(TAG, "$logPrefix 第 ${index+1} 行使用右对齐，处理后: ${processedLine.take(20)}${if(processedLine.length > 20) "..." else ""}")
                    }
                    line.startsWith("[L]") -> {
                        formattedContent.append(RASTER_MODE_ALIGN_LEFT)
                        val processedLine = processRasterLine(line.substring(3))
                        formattedContent.append(processedLine)
                        Log.d(TAG, "$logPrefix 第 ${index+1} 行使用左对齐，处理后: ${processedLine.take(20)}${if(processedLine.length > 20) "..." else ""}")
                    }
                    else -> {
                        formattedContent.append(RASTER_MODE_ALIGN_LEFT)
                        val processedLine = processRasterLine(line)
                        formattedContent.append(processedLine)
                        Log.d(TAG, "$logPrefix 第 ${index+1} 行使用默认左对齐，处理后: ${processedLine.take(20)}${if(processedLine.length > 20) "..." else ""}")
                    }
                }
                formattedContent.append("\n")
                
                // 每50行记录一次处理进度
                if (index % 50 == 49 || index == lines.size - 1) {
                    Log.d(TAG, "$logPrefix 已处理 ${index + 1}/${lines.size} 行")
                }
            }

            // 5. 打印内容
            Log.d(TAG, "$logPrefix 开始打印格式化后内容，总长度: ${formattedContent.length}字符")
            val finalContent = formattedContent.toString()
            ...
            for (i in contentBytes.indices step chunkSize) {
                ...
                Log.d(TAG, "$logPrefix 打印段 $chunkNum/$totalChunks, 字节范围: $i - ${end-1}, 大小: ${chunk.size}字节")
                
                // 记录每个分段的前30个字节，用于调试
                val chunkHex = chunk.take(30).joinToString(" ") { 
                    String.format("%02X", it.toInt() and 0xFF) 
                }
                Log.d(TAG, "$logPrefix 段 $chunkNum 前30字节(HEX): $chunkHex${if(chunk.size > 30) "..." else ""}")
                
                try {
                    currentPrinter?.printAsync(String(chunk))?.await()
                    Log.d(TAG, "$logPrefix 段 $chunkNum/$totalChunks 打印成功")
                } catch (e: Exception) {
                    Log.e(TAG, "$logPrefix 段 $chunkNum/$totalChunks 打印失败: ${e.message}")
                    Log.e(TAG, "$logPrefix 异常详情: ${e.stackTraceToString().take(500)}...")
                }
                ...
                if (chunkNum % 5 == 0) {
                    try {
                        val chunkStatus = currentPrinter?.getStatusAsync()?.await()
                        Log.d(TAG, "$logPrefix 完成 $chunkNum/$totalChunks 段后状态: $chunkStatus")
                    } catch (e: Exception) {
                        Log.w(TAG, "$logPrefix 获取打印机状态失败: ${e.message}")
                    }
                }
            ...
            try {
                ...
                Log.d(TAG, "$logPrefix 走纸和切纸命令发送成功，命令HEX: ${RASTER_MODE_CUT.toByteArray().joinToString(" ") { String.format("%02X", it.toInt() and 0xFF) }}")
            } catch (e: Exception) {
            ...
            try {
                ...
                Log.e(TAG, "$logPrefix 当前的代码已经替换完毕，我现在将编辑另一个部分，增加 processRasterLine 方法的详细日志信息。")
            }
    ...
    private fun processRasterLine(line: String): String {
        ...
        Log.d(TAG, "【光栅处理】处理文本行: ${if(line.length > 30) line.substring(0, 30) + "..." else line}")
        
        // 原始行的十六进制表示
        val originalHex = line.toByteArray().take(20).joinToString(" ") { 
            String.format("%02X", it.toInt() and 0xFF) 
        }
        Log.d(TAG, "【光栅处理】原始行前20字节(HEX): $originalHex${if(line.toByteArray().size > 20) "..." else ""}")
        
        // 处理加粗标签
        var text = line
        if (text.contains("<b>") || text.contains("</b>")) {
            Log.d(TAG, "【光栅处理】检测到加粗标签，位置: <b>=${text.indexOf("<b>")}, </b>=${text.indexOf("</b>")}")
            
            // 详细记录替换过程
            val boldStartCmd = RASTER_MODE_BOLD_ON
            val boldEndCmd = RASTER_MODE_BOLD_OFF
            
            Log.d(TAG, "【光栅处理】加粗开始命令(HEX): ${boldStartCmd.toByteArray().joinToString(" ") { String.format("%02X", it.toInt() and 0xFF) }}")
            Log.d(TAG, "【光栅处理】加粗结束命令(HEX): ${boldEndCmd.toByteArray().joinToString(" ") { String.format("%02X", it.toInt() and 0xFF) }}")
            
            text = text.replace("<b>", boldStartCmd)
            text = text.replace("</b>", boldEndCmd)
            
            // 记录替换后的十六进制字符
            val hexText = text.toByteArray().take(40).joinToString(" ") { 
                String.format("%02X", it.toInt() and 0xFF) 
            }
            Log.d(TAG, "【光栅处理】替换加粗标签后前40字节(HEX): $hexText${if(text.toByteArray().size > 40) "..." else ""}")
        }
        
        // 检查其他可能的格式化标签
        if (text.contains("<") || text.contains(">")) {
            Log.d(TAG, "【光栅处理】警告：可能存在未处理的格式化标签: ${text.take(50)}${if(text.length > 50) "..." else ""}")
        }
        
        processedLine.append(text)
        Log.d(TAG, "【光栅处理】处理完成，长度从 ${line.length} 变为 ${processedLine.length} 字符")
