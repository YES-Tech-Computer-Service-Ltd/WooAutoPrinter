...
class DefaultOrderPrintTemplate @Inject constructor(
    ...
    fun createTestOrder(config: PrinterConfig): Order {
        ...
        // 根据纸张宽度选择不同的测试内容
        return when (config.paperWidth) {
            PrinterConfig.PAPER_WIDTH_57MM -> createTestOrder58mm(currentTime, dateFormat)
            PrinterConfig.PAPER_WIDTH_80MM -> createTestOrder80mm(currentTime, dateFormat)
            else -> createTestOrderDefault(currentTime, dateFormat)
        }
    }
    
    private fun createTestOrder58mm(currentTime: Date, dateFormat: SimpleDateFormat): Order {
        return Order(
            id = 999999L,
            number = "TEST58-${System.currentTimeMillis()}",
            status = "processing",
            ...
            notes = "58mm printer test order",
            items = listOf(
                Order.OrderItem(
                    id = 1,
                    name = "Chicken Curry",
                    quantity = 2.toBigDecimal(),
                    price = 19.toBigDecimal(),
                    options = emptyList()
                ),
                Order.OrderItem(
                    id = 2,
    ...
    private fun createTestOrder80mm(currentTime: Date, dateFormat: SimpleDateFormat): Order {
        return Order(
            id = 999998L,
            number = "TEST80-${System.currentTimeMillis()}",
            status = "processing", 
            dateCreated = currentTime,
            ...
            total = "$177.48", 
            paymentMethod = "Credit Card",
            notes = "80mm thermal printer comprehensive test order.\nThis tests multiple features including alignment, formatting, and character encoding.",
            items = listOf(
                Order.OrderItem(
                    id = 1,
                    name = "Braised Pork Set",
                    quantity = 2.toBigDecimal(),
                    price = 45.toBigDecimal(),
                    options = listOf(
                        Order.OrderItemOption(
                            name = "Side dishes",
                            value = "Rice, Vegetables, Soup"
                        )
                    )
                ),
                Order.OrderItem(
                    ...
                    quantity = 1.toBigDecimal(),
                    price = 32.toBigDecimal(),
                    options = listOf(
                        Order.OrderItemOption(
                            name = "Spice level",
                            value = "Medium, Less oil"
                        )
                    )
                ),
                Order.OrderItem(
            ...
            woofoodInfo = Order.WoofoodInfo(
                isDelivery = true,
                deliveryAddress = "123 Business Avenue, Unit 100",
                deliveryFee = "$5.00",
                tip = "$15.00"
            )
        )
    ...
    private fun createTestOrderDefault(currentTime: Date, dateFormat: SimpleDateFormat): Order {
        return Order(
            id = 999997L,
            ...
            total = "$65.00",
            paymentMethod = "Credit Card",
            notes = "Default printer test order",
            items = listOf(
                Order.OrderItem(
                    id = 1,
                    name = "Test Item A",
