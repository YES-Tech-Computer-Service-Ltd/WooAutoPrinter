...
@Composable
fun PrinterDetailsDialogContent(
    viewModel: SettingsViewModel = hiltViewModel(),
    onClose: () -> Unit,
    printerId: String? = null,
    onSaved: ((PrinterConfig) -> Unit)? = null
) {
                ...
                navigationIcon = {
                    IconButton(onClick = { 
                        if (isNewPrinter && hasSelectedDevice) {
                            ...
                            Log.d("PrinterDetails", "关闭对话框")
                            onClose()
                        }
                    }) {
                        Icon(
                            imageVector = if (isNewPrinter && hasSelectedDevice) 
                                Icons.AutoMirrored.Filled.ArrowBack else Icons.Default.Close,
                            contentDescription = if (isNewPrinter && hasSelectedDevice) 
                                stringResource(id = R.string.back) else stringResource(id = R.string.close)
                        )
                    }
                },
        ...
        if (isNewPrinter && !hasSelectedDevice) {
            ...
            ConfigurationScreen(
                ...
                onSave = {
                    ...
                    // 通知调用者配置已保存
                    onSaved?.invoke(updatedConfig)
                    
                    // 关闭对话框
                    Log.d("PrinterDetails", "保存完成，关闭对话框")
                    onClose()
                },
                viewModel = viewModel,
...
/**
 * 保留原有 PrinterDetailsScreen 组件以保持向后兼容，但使用新的对话框实现
 */
@RequiresApi(Build.VERSION_CODES.S)
@Composable
