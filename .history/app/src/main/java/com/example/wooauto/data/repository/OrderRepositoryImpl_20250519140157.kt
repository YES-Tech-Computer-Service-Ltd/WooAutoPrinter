...
//                Log.d("OrderRepositoryImpl", "【应用启动】数据库中找到 ${printedOrderIds.size} 个已打印订单，将保留其打印状态")
            ...
            // API返回订单后，先转换为领域模型
            val apiOrders = response.map { it.toOrder() }

            // 1. 获取本地所有订单的打印状态
            val localOrders = orderDao.getAllOrders().first()
            val printedMap = localOrders.associateBy({ it.id }, { it.isPrinted })

            // 2. 合并打印状态
            val mergedOrders = apiOrders.map { order ->
                val isPrinted = printedMap[order.id] ?: false
                order.copy(isPrinted = isPrinted)
            }

            // 3. 后续逻辑全部用mergedOrders替换apiOrders/response
            // 转换为领域模型并保留已读状态
            val orders = mergedOrders.map { order ->
                ...
                if (entity.id in readOrderIds) {
                    ...
                    // 然后检查数据库中的现有状态
                    val existingOrder = existingOrderMap[entity.id]
                    if (existingOrder != null) {
                        ...
                        if (entity.dateCreated.time < thirtyDaysAgo) {
//                            Log.d("OrderRepositoryImpl", "订单 #${entity.number} (ID=${entity.id}) 是较旧订单(>30天)，自动标记为已读")
                            entity.copy(isRead = true)
                        } else {
                            entity
                        }
...
//                Log.d("OrderRepositoryImpl", "执行全量更新：${orders.size} 个订单")
            } else if (status != null) {
                // 按状态更新：只删除指定状态的旧数据
//                Log.d("OrderRepositoryImpl", "更新状态为 '$status' 的订单：${orders.size} 个")
                orderDao.deleteOrdersByStatus(status)
            }
            
            // 插入更新后的订单
            val orderEntities = orders.map { OrderMapper.mapDomainToEntity(it) }
            ...
            if (status == null) {
                ...
                val statusOrdersFromResponse = mergedOrders.filter { it.status == status }
            ...
            return@withContext Result.success(mergedOrders)
        } catch (e: Exception) {
...
//                Log.d("OrderRepositoryImpl", "【轮询刷新】更新 ${entitiesWithPrintStatus.size} 个processing订单")
                val orderEntities = entitiesWithPrintStatus.map { OrderMapper.mapDomainToEntity(it) }
                orderDao.insertOrders(orderEntities)
...
//        Log.d("OrderRepositoryImpl", "【状态分组】所有状态: ${groupedOrders.keys.joinToString()}")
        ...
        val printedCountByStatus = mutableMapOf<String, Int>()
        
        // 首先从数据库获取所有标记为已打印的订单ID
        runBlocking {
            try {
...
//                    Log.d("OrderRepositoryImpl", "【打印状态同步】数据库中没有已打印订单")
        ...
        // 使用更新后的内存状态重新分组
        val updatedGroupedOrders = cachedOrders.groupBy { it.status }
