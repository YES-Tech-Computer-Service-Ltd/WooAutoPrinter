...
class ProductRepositoryImpl @Inject constructor(
    ...
    override suspend fun refreshProducts(categoryId: Long?): Result<List<Product>> {
        return try {
            ...
            try {
                ...
                Log.e("ProductRepositoryImpl", "获取API配置时出错", e)
            }
    ...
    override suspend fun testConnection(config: WooCommerceConfig): Boolean = withContext(Dispatchers.IO) {
        ...
        try {
            // 使用临时创建的API实例，确保使用传入的config参数
            val tempApiFactory = WooCommerceApiFactory.getInstance(config)
            val tempApi = tempApiFactory.createApi(config)
            
            // 尝试获取产品列表
            val products = tempApi.getProducts(1, 1)
