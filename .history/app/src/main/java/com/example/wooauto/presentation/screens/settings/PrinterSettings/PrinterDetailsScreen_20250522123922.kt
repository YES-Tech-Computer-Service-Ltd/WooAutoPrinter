...
fun PrinterDetailsScreen(
    ...
    // State to force showing scanning UI on initial entry for a new printer
    var forceShowScanningInitially by remember(currentStep, isNewPrinter) {
        mutableStateOf(isNewPrinter && currentStep == SetupStep.SELECT_DEVICE)
    }
    
    // 页面加载时自动扫描打印机列表（如果是添加新打印机或进入设备选择步骤时）
    LaunchedEffect(currentStep, isNewPrinter) {
        if (isNewPrinter && currentStep == SetupStep.SELECT_DEVICE) {
            // It's important to set forceShowScanningInitially true *before* calling scanPrinters,
            // though remember might have already set it for the current composition.
            forceShowScanningInitially = true 
            viewModel.scanPrinters()
        } else {
            // Reset if we are not in the initial new printer selection step
            forceShowScanningInitially = false
        }
    }
    
    // If the ViewModel's scanning state catches up, or we're no longer in the initial forced state,
    // turn off the forcing flag.
    if (forceShowScanningInitially && (isScanning || !(isNewPrinter && currentStep == SetupStep.SELECT_DEVICE))) {
        forceShowScanningInitially = false
    }
        ...
        when (currentStep) {
            SetupStep.SELECT_DEVICE -> {
                ...
                DeviceSelectionContent(
                    ...
                    isScanning = if (forceShowScanningInitially) true else isScanning,
                    onScanClick = { 
                        forceShowScanningInitially = false
                        viewModel.scanPrinters() 
                    },
...
private fun DeviceSelectionContent(
    ...
    Column(
        ...
        if (!isScanning && availablePrinters.isEmpty()) {
            ...
            // 用Box替代直接调用DeviceList，因为Column内部不能在LazyColumn上使用weight
            Box(
                modifier = Modifier
                    .fillMaxWidth()
                    .weight(1f)
