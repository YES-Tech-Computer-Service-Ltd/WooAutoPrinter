...
fun WooAppBar(
    ...
    // 生成层级标题（一级 - 二级），仅针对 settings/{section}/{sub}
    val compositeTitle: String? = run {
        if (currentRoute.startsWith("settings/")) {
            val parts = currentRoute.removePrefix("settings/").split('/')
            if (parts.size >= 2) {
                val sectionKey = parts[0]
                val subKey = parts[1]
                val sectionRes = AppNavConfig.subEntriesForRoute(NavigationItem.Settings.route)
                    .find { it.route == sectionKey }?.titleResId ?: R.string.settings
                // 二级标题：根据 subKey 做简单映射，目前只识别 language
                val subRes = when (subKey) {
                    "language" -> R.string.language
                    else -> null
                }
                if (subRes != null) stringResource(sectionRes) + " - " + stringResource(subRes) else null
            } else null
        } else null
    }
    when (currentRoute) {
