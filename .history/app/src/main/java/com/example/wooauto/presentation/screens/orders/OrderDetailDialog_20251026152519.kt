                    ...
                    actions = {
                        ...
                        if (BuildConfig.DEBUG) {
                            try {
                                android.util.Log.d(
                                    "OrderDebug",
                                    "UI Detail TopBar -> order=#${displayOrder.number}, created=${java.text.SimpleDateFormat("yyyy-MM-dd HH:mm", java.util.Locale.getDefault()).format(displayOrder.dateCreated)}, parsedDate=${scheduledDateStr}, time=${timeText}, statusText=${status?.text}"
                                )
                            } catch (_: Exception) { }
                        }
                        if (status != null) {
                ...
                Box(
                    ...
                    Column(
                        ...
                        displayOrder.woofoodInfo?.let { wooFoodInfo ->
                            ...
                            if (BuildConfig.DEBUG && displayOrder.number == "1700") {
                                ...
                                LaunchedEffect(displayOrder.id) {
                                    try {
                                        rawMeta = repo.getRawOrderMetadata(displayOrder.id)
                                    } catch (_: Exception) { rawMeta = null }
                                }
                                OrderDetailRow(
                                    label = "Meta",
    ...
    return try {
        ...
        if (BuildConfig.DEBUG) {
            try {
                android.util.Log.d(
                    "OrderDebug",
                    "extractScheduledDateFromNotes: noteLen=${note.length} preview=\n" + note.take(400)
                )
            } catch (_: Exception) { }
        }
        for (key in keys) {
            ...
            if (match != null && match.groupValues.size > 1) {
                ...
                if (BuildConfig.DEBUG) {
                    try {
                        android.util.Log.d(
                            "OrderDebug",
                            "extractScheduledDateFromNotes: key='${key}' raw='${raw}' formatted='${formatted}'"
                        )
                    } catch (_: Exception) { }
                }
        ...
        if (BuildConfig.DEBUG) {
            try { android.util.Log.d("OrderDebug", "extractScheduledDateFromNotes: no date matched") } catch (_: Exception) { }
        }
        null
    } catch (e: Exception) {
        null
