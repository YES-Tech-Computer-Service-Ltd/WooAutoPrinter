...
import com.starmicronics.starxpandcommand.StarXpandCommandBuilder
import com.starmicronics.starxpandcommand.DocumentBuilder
import com.starmicronics.starxpandcommand.PrinterBuilder
import com.starmicronics.starxpandcommand.MagnificationParameter
import com.starmicronics.starxpandcommand.printer.Alignment
import com.starmicronics.starxpandcommand.printer.CutType
import com.starmicronics.starxpandcommand.printer.InternationalCharacterType
import com.starmicronics.starxpandcommand.printer.QRCodeLevel
import com.starmicronics.starxpandcommand.printer.BarcodeSymbology
...
class StarPrinterHandler(private val context: Context) {
    companion object {
        ...
        private val TSP100_MODELS = listOf(
            "tsp100", "tsp143", "tsp100iii", "tsp143iii", "tsp100iiibi", "tsp143iiibi", "tsp143iv"
        )
        ...
        // TSP100III光栅模式命令集 - 更新为符合TSP100III规范的命令
        ...
        private val RASTER_MODE_START = byteArrayOf(
            ...
            0x1B, 0x1D, 0x57, 0x40, 0x01,  // ESC GS W 64 1 - 设置打印区域宽度(384点)
            0x1B, 0x1D, 0x41, 0x01,        // ESC GS A 1 - 启用光栅缓冲
            0x1B, 0x1D, 0x4D, 0x01,        // ESC GS M 1 - 设置打印浓度
            0x1B, 0x1D, 0x79, 0x30, 0x30   // ESC GS y 0 0 - 设置光栅打印速度
        ).toByteString()

        private val RASTER_MODE_CLEAR = byteArrayOf(
            0x18,        // CAN - 清除缓冲区
            0x1B, 0x40   // ESC @ - 初始化打印机
        ).toByteString()
    ...
    suspend fun printTest(config: PrinterConfig): Boolean {
        ...
        return printTestWithCommandBuilder()
    }
    ...
    suspend fun printWithRasterMode(content: String): Boolean {
        ...
        try {
            ...
            try {
                ...
                Log.d(TAG, "$logPrefix 缓冲区清除完成")
                delay(300)
            } catch (e: Exception) {
                Log.e(TAG, "$logPrefix 清除缓冲区失败: ${e.message}", e)
                // 继续执行
            }

            // 检查打印机状态
            try {
                val statusBeforeInit = currentPrinter?.getStatusAsync()?.await()
                Log.d(TAG, "$logPrefix 初始化前状态: $statusBeforeInit")
            } catch (e: Exception) {
                Log.w(TAG, "$logPrefix 获取初始状态失败: ${e.message}")
            }
            ...
            try {
                // 记录初始化命令
                val initHex = RASTER_MODE_INIT.toByteArray().joinToString("") { 
                    String.format("%02X ", it.toInt() and 0xFF) 
                }
                Log.d(TAG, "$logPrefix 初始化命令(HEX): $initHex")
                
                currentPrinter?.printAsync(RASTER_MODE_INIT)?.await()
                Log.d(TAG, "$logPrefix 光栅模式初始化完成")
                delay(300)
            } catch (e: Exception) {
                Log.e(TAG, "$logPrefix 初始化光栅模式失败: ${e.message}", e)
                // 继续执行
            }

            // 3. 开始光栅模式
            Log.d(TAG, "$logPrefix 配置光栅模式参数")
            try {
                // 记录配置命令
                val startHex = RASTER_MODE_START.toByteArray().joinToString("") { 
                    String.format("%02X ", it.toInt() and 0xFF) 
                }
                Log.d(TAG, "$logPrefix 光栅模式配置命令(HEX): $startHex")
                
                currentPrinter?.printAsync(RASTER_MODE_START)?.await()
                Log.d(TAG, "$logPrefix 光栅模式参数配置完成")
                delay(300)
            } catch (e: Exception) {
                Log.e(TAG, "$logPrefix 配置光栅模式参数失败: ${e.message}", e)
                // 继续执行
            }
            ...
            try {
                val status = currentPrinter?.getStatusAsync()?.await()
                Log.d(TAG, "$logPrefix 配置后状态: $status")
                
                if (status?.hasError == true || status?.paperEmpty == true) {
                    Log.e(TAG, "$logPrefix 打印机状态异常: $status")
                }
            } catch (e: Exception) {
                Log.w(TAG, "$logPrefix 获取状态失败: ${e.message}")
                // 继续执行
            }

            // 4. 格式化并打印内容
            Log.d(TAG, "$logPrefix 开始打印内容...")
            
            // 打印标题
            val header = StringBuilder()
            header.append(RASTER_MODE_ALIGN_CENTER)
            header.append("*** 订单打印 ***\n\n")
            currentPrinter?.printAsync(header.toString())?.await()
            delay(100)
            
            // 分行处理内容
            val lines = content.split("\n")
            Log.d(TAG, "$logPrefix 内容行数: ${lines.size}")
            
            // 记录内容格式分析
            var hasAlignCommands = false
            var hasBoldCommands = false
            lines.take(10).forEach { line ->
                if (line.contains("[C]") || line.contains("[L]") || line.contains("[R]")) {
                    hasAlignCommands = true
                }
                if (line.contains("<b>") || line.contains("</b>")) {
                    hasBoldCommands = true
                }
            }
            Log.d(TAG, "$logPrefix 内容格式分析 - 对齐命令: $hasAlignCommands, 加粗命令: $hasBoldCommands")
            ...
            for ((index, line) in lines.withIndex()) {
                ...
                when {
                    line.startsWith("[C]") -> {
                        formattedLine.append(RASTER_MODE_ALIGN_CENTER)
                        formattedLine.append(line.substring(3))
                        Log.d(TAG, "$logPrefix 处理居中行[$index]: ${line.substring(3)}")
                    }
                    line.startsWith("[R]") -> {
                        formattedLine.append(RASTER_MODE_ALIGN_RIGHT)
                        formattedLine.append(line.substring(3))
                        Log.d(TAG, "$logPrefix 处理右对齐行[$index]: ${line.substring(3)}")
                    }
                    line.startsWith("[L]") -> {
                        formattedLine.append(RASTER_MODE_ALIGN_LEFT)
                        formattedLine.append(line.substring(3))
                        Log.d(TAG, "$logPrefix 处理左对齐行[$index]: ${line.substring(3)}")
                    }
                    else -> {
                        formattedLine.append(RASTER_MODE_ALIGN_LEFT)
                        formattedLine.append(line)
                        if (index < 5 || index > lines.size - 5) { // 只记录开头和结尾的几行
                            Log.d(TAG, "$logPrefix 处理普通行[$index]: $line")
                        }
                    }
                }
                ...
                // 处理加粗等格式
                val processedLine = formattedLine.toString()
                    .replace("<b>", RASTER_MODE_BOLD_ON)
                    .replace("</b>", RASTER_MODE_BOLD_OFF)
                
                try {
                    currentPrinter?.printAsync(processedLine)?.await()
                    if (index % 20 == 0) { // 每20行记录一次进度
                        Log.d(TAG, "$logPrefix 已处理 ${index+1}/${lines.size} 行")
                    }
                    delay(10) // 短暂延迟防止缓冲区溢出
                } catch (e: Exception) {
                    Log.e(TAG, "$logPrefix 打印第${index+1}行时出错: ${e.message}")
                    // 继续尝试打印其他行
                }
            }
            
            // 5. 结束打印
            Log.d(TAG, "$logPrefix 完成打印内容")
            
            // 添加页脚
            val footer = StringBuilder()
            footer.append(RASTER_MODE_ALIGN_CENTER)
            footer.append("\n--- 打印完成 ---\n\n\n")
            currentPrinter?.printAsync(footer.toString())?.await()
            delay(100)
     ...
     * 使用非常基础的命令测试打印机
     */
    suspend fun printTSP100IIIBasicTest(): Boolean {
        val logPrefix = "【超简单测试】"
        try {
            if (currentPrinter == null) {
                Log.e(TAG, "$logPrefix 打印机未连接")
            ...
            Log.d(TAG, "$logPrefix 开始超简单命令测试")
            
            // 1. 清除缓冲区
            val clearCommand = "\u0018"  // CAN - 清除缓冲区
            currentPrinter?.printAsync(clearCommand)?.await()
            delay(300)
            
            // 2. 发送初始化命令
            val initCommand = "\u001b\u0040" +     // ESC @ - 初始化打印机
                            "\u001b\u0074\u0001"   // ESC t 1 - 设置代码页PC437
            Log.d(TAG, "$logPrefix 发送初始化命令: ${initCommand.toByteArray().joinToString(" ") { String.format("%02X", it.toInt() and 0xFF) }}")
            currentPrinter?.printAsync(initCommand)?.await()
            delay(500)
            
            // 3. 发送居中对齐命令
            val alignCommand = "\u001b\u0061\u0001"  // ESC a 1 - 居中对齐
            currentPrinter?.printAsync(alignCommand)?.await()
            delay(200)
            
            // 4. 发送加粗命令
            val boldCommand = "\u001b\u0045\u0001"  // ESC E 1 - 加粗开启
            currentPrinter?.printAsync(boldCommand)?.await()
            delay(200)
            
            // 5. 发送最简单的文本
            val testText = "*** 超简单测试 ***\n\n这是一个基本文本测试\n\n"
            Log.d(TAG, "$logPrefix 发送基本文本: $testText")
            currentPrinter?.printAsync(testText)?.await()
            delay(500)
            
            // 6. 取消加粗
            val boldOffCommand = "\u001b\u0046"  // ESC F - 加粗关闭
            currentPrinter?.printAsync(boldOffCommand)?.await()
            delay(200)
            ...
            val feedAndCut = "\n\n\n" +
                           ...
                           "\u001d\u0056\u0001"    // GS V 1 - 切纸
            Log.d(TAG, "$logPrefix 发送走纸和切纸命令")
            currentPrinter?.printAsync(feedAndCut)?.await()
            delay(500)
            
            Log.d(TAG, "$logPrefix 测试完成")
            return true
        } catch (e: Exception) {
            Log.e(TAG, "$logPrefix 测试失败: ${e.message}")
            return false
        }
    }

    /**
     * 使用 StarXpand CommandBuilder 打印文本
     * 这是推荐的打印方式,可以确保打印指令被正确识别
     */
    suspend fun printWithCommandBuilder(content: String): Boolean {
        val logPrefix = "【CommandBuilder打印】"
        try {
            if (currentPrinter == null) {
                Log.e(TAG, "$logPrefix 打印机未连接")
                return false
            ...
            Log.d(TAG, "$logPrefix 开始使用 CommandBuilder 打印")
            
            // 创建 CommandBuilder 实例
                            ...
                            content.split("\n").forEach { line ->
                                when {
                                    line.startsWith("[C]") -> {
                                        ...
                                        actionPrintText(line.substring(3) + "\n")
                                    }
                                    line.startsWith("[R]") -> {
    ...
    /**
     * 使用 StarXpand CommandBuilder 打印测试页
     */
    suspend fun printTestWithCommandBuilder(): Boolean {
        val logPrefix = "【CommandBuilder测试】"
        try {
            if (currentPrinter == null) {
                Log.e(TAG, "$logPrefix 打印机未连接")
                return false
                ...
                DocumentBuilder()
                    .addPrinter(
                        PrinterBuilder().apply {
                            ...
                            // 左对齐打印信息
                            styleAlignment(Alignment.Left)
                            actionPrintText("打印机型号: ${currentPrinter?.information?.model ?: "未知"}\n")
                            actionPrintText("时间: ${java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(java.util.Date())}\n\n")
                            
                            // 测试不同样式
                            actionPrintText("样式测试:\n")
                            actionPrintText("1. 标准文本\n")
                            
                            styleBold(true)
                            actionPrintText("2. 加粗文本\n")
                            styleBold(false)
                            
                            styleAlignment(Alignment.Center)
                            actionPrintText("3. 居中对齐\n")
                            
                            styleAlignment(Alignment.Right)
                            actionPrintText("4. 右对齐\n")
                            
                            styleAlignment(Alignment.Left)
                            styleUnderLine(true)
                            actionPrintText("5. 下划线文本\n")
                            styleUnderLine(false)
                            
                            // 打印分隔线
                            styleAlignment(Alignment.Center)
                            actionPrintText("\n----------------\n")
                            actionPrintText("测试完成\n\n")
            ...
            Log.d(TAG, "$logPrefix 测试页打印完成")
            return true
            
        } catch (e: Exception) {
            Log.e(TAG, "$logPrefix 测试页打印失败: ${e.message}")
            Log.e(TAG, "$logPrefix 异常堆栈: ${e.stackTraceToString()}")
            return false
        }
    }
     ...
     * 使用多种不同的光栅命令组合测试打印机
     */
    suspend fun printTSP100IIIRasterTest(): Boolean {
        val logPrefix = "【TSP100光栅测试】"
        return printTestWithCommandBuilder()  // 使用新版命令构建器方式
    }
    ...
    /**
     * 使用StarXpand SDK打印二维码
     * @param content 二维码内容
     * @param cellSize 二维码单元格大小
     * @param alignment 对齐方式
     * @return 打印是否成功
     */
    suspend fun printQRCode(content: String, cellSize: Int = 8, alignment: Alignment = Alignment.Center): Boolean {
        val logPrefix = "【打印二维码】"
        try {
            if (currentPrinter == null) {
                Log.e(TAG, "$logPrefix 打印机未连接")
            ...
            Log.d(TAG, "$logPrefix 开始打印二维码: $content")
                ...
                DocumentBuilder()
                    .addPrinter(
                        PrinterBuilder().apply {
                            ...
                            // 打印二维码
                            actionPrintQRCode(
                                com.starmicronics.starxpandcommand.printer.QRCodeParameter(
                                    content
                                ).apply {
    ...
    /**
     * 使用StarXpand SDK打印条形码
     * @param content 条形码内容
     * @param symbology 条形码类型
     * @return 打印是否成功
     */
    suspend fun printBarcode(
        ...
        try {
            ...
            Log.d(TAG, "$logPrefix 开始打印条形码: $content")
                ...
                DocumentBuilder()
                    .addPrinter(
                        PrinterBuilder().apply {
                            ...
                            // 打印条形码
                            actionPrintBarcode(
                                com.starmicronics.starxpandcommand.printer.BarcodeParameter(
                                    content,
                                    symbology
                                ).apply {
                                    setHeight(5.0)  // 设置高度
                                    setBarDots(3)   // 设置条码宽度
                                    setPrintHri(true) // 打印可读内容
                                }
                            )
     ...
     * @return 打印是否成功
     */
    suspend fun printImage(bitmap: android.graphics.Bitmap, width: Int = 406): Boolean {
        val logPrefix = "【打印图片】"
        try {
            if (currentPrinter == null) {
                Log.e(TAG, "$logPrefix 打印机未连接")
            ...
            Log.d(TAG, "$logPrefix 开始打印图片, 尺寸: ${bitmap.width}x${bitmap.height}, 打印宽度: $width")
                ...
                DocumentBuilder()
                    .addPrinter(
                        PrinterBuilder().apply {
                            ...
                            // 打印图片
                            actionPrintImage(
                                com.starmicronics.starxpandcommand.printer.ImageParameter(
                                    bitmap,
                                    width
                                )
                            )
