...
class StarPrinterHandler(private val context: Context) {
    ...
    suspend fun printWithRasterMode(content: String): Boolean {
        ...
        try {
            ...
            // 检查打印机型号
            val currentModel = currentPrinter?.information?.model ?: "未知型号"
            val isTSP100Series = currentModel.contains("TSP100", ignoreCase = true) || 
                                lastConnectedModel.contains("TSP100", ignoreCase = true)
            
            Log.d(TAG, "$logPrefix =============== 开始光栅模式打印 ===============")
            Log.d(TAG, "$logPrefix 打印内容长度: ${content.length}字符")
            Log.d(TAG, "$logPrefix 内容预览: ${content.take(100)}${if(content.length > 100) "..." else ""}")
            Log.d(TAG, "$logPrefix 打印机型号: $currentModel, 是否TSP100系列: $isTSP100Series")
            
            // 清理前一次打印可能的剩余数据
            Log.d(TAG, "$logPrefix 清除打印缓冲区")
            currentPrinter?.printAsync("\u0018")?.await() // CAN - 清除缓冲区
            delay(200)
            ...
            val initialStatus = try {
                ...
                val statusDesc = when {
                    status == null -> "状态未知"
                    status.hasError -> "有错误: ${status.toString()}"
                    status.coverOpen -> "纸仓盖打开"
                    status.paperEmpty -> "缺纸"
                    else -> "正常"
                }
                Log.d(TAG, "$logPrefix 初始打印机状态: $statusDesc")
                status
            } catch (e: Exception) {
                Log.e(TAG, "$logPrefix 获取初始状态失败: ${e.message}")
                null
            }
            
            // 拆分内容为行
            val lines = content.lines()
            Log.d(TAG, "$logPrefix 内容共 ${lines.size} 行")
            
            // 预览前十行内容
            lines.take(10).forEachIndexed { index, line ->
                Log.d(TAG, "$logPrefix 第${index+1}行预览: ${line.take(50)}${if(line.length > 50) "..." else ""}")
            }
            if (lines.size > 10) {
                Log.d(TAG, "$logPrefix (仅显示前10行，共${lines.size}行)")
            }
            
            // 初始化打印机
            val initCommand = StringBuilder()
                .append("\u001B\u0040")                 // ESC @ - 初始化打印机
                .append("\u001B\u001D\u0074\u0001")     // ESC GS t 1 - 设置代码页
                .append("\u001B\u001D\u0061\u0001")     // ESC GS a 1 - 启用自动状态回传
                .toString()
            
            Log.d(TAG, "$logPrefix 发送初始化命令: ${bytesToHex(initCommand.toByteArray())}")
            currentPrinter?.printAsync(initCommand)?.await()
            delay(200)
            
            // 进入光栅模式
            val startRasterCmd = StringBuilder()
                .append("\u001B\u001D\u0073\u0000\u0030")  // ESC GS s 0 0 - 设置光栅打印质量
                .append("\u001B\u001D\u004C\u0000\u0000")  // ESC GS L 0 0 - 设置左边距
                .append("\u001B\u001D\u0057\u0040\u0001")  // ESC GS W 64 1 - 设置打印区域宽度
                .append("\u001B\u001D\u0041\u0001")        // ESC GS A 1 - 启用光栅缓冲
                .toString()
            
            Log.d(TAG, "$logPrefix 发送光栅模式命令: ${bytesToHex(startRasterCmd.toByteArray())}")
            currentPrinter?.printAsync(startRasterCmd)?.await()
            delay(200)
            
            // 设置默认对齐方式为左对齐
            val defaultAlign = "\u001B\u001D\u0061\u0000" // ESC GS a 0 - 左对齐
            Log.d(TAG, "$logPrefix 设置默认对齐方式(左对齐): ${bytesToHex(defaultAlign.toByteArray())}")
            currentPrinter?.printAsync(defaultAlign)?.await()
            ...
            // 保存所有行的字节数据
            val allLineData = mutableListOf<ByteArray>()
            var totalBytes = 0
            
            // 处理每一行内容
            Log.d(TAG, "$logPrefix 开始处理各行内容...")
            lines.forEachIndexed { lineIndex, line ->
                val originalLine = line.trim()
                var processedLine = originalLine
                var lineBytes: ByteArray? = null
                var lineCommand: String? = null
                
                // 检查行对齐方式
                when {
                    processedLine.startsWith("[L]") -> {
                        if (currentAlignment != "LEFT") {
                            Log.d(TAG, "$logPrefix 行${lineIndex+1}: 切换到左对齐")
                            lineCommand = "\u001B\u001D\u0061\u0000" // ESC GS a 0 - 左对齐
                            currentAlignment = "LEFT"
                        }
                        processedLine = processedLine.substring(3)
                    }
                    processedLine.startsWith("[C]") -> {
                        if (currentAlignment != "CENTER") {
                            Log.d(TAG, "$logPrefix 行${lineIndex+1}: 切换到居中对齐")
                            lineCommand = "\u001B\u001D\u0061\u0001" // ESC GS a 1 - 居中对齐 
                            currentAlignment = "CENTER"
                        }
                        processedLine = processedLine.substring(3)
                    }
                    processedLine.startsWith("[R]") -> {
                        if (currentAlignment != "RIGHT") {
                            Log.d(TAG, "$logPrefix 行${lineIndex+1}: 切换到右对齐")
                            lineCommand = "\u001B\u001D\u0061\u0002" // ESC GS a 2 - 右对齐
                ...
                // 如果需要改变对齐方式，添加命令
                if (lineCommand != null) {
                    Log.d(TAG, "$logPrefix 行${lineIndex+1}: 发送对齐命令: ${bytesToHex(lineCommand.toByteArray())}")
                    currentPrinter?.printAsync(lineCommand)?.await()
                }
                
                // 处理加粗标签
                val hasBoldTag = processedLine.contains("<b>") && processedLine.contains("</b>")
                if (hasBoldTag) {
                    Log.d(TAG, "$logPrefix 行${lineIndex+1}: 检测到加粗标签")
                    ...
                    while (i < processedLine.length) {
                        if (i + 2 < processedLine.length && processedLine.substring(i, i + 3) == "<b>") {
                            // 进入加粗模式
                            if (!boldMode) {
                                val boldOn = "\u001B\u001D\u0045\u0001" // ESC GS E 1 - 加粗开启
                                Log.d(TAG, "$logPrefix 行${lineIndex+1}: 开启加粗: ${bytesToHex(boldOn.toByteArray())}")
                                currentPrinter?.printAsync(boldOn)?.await()
                                boldMode = true
                            }
                            i += 3
                        } else if (i + 3 < processedLine.length && processedLine.substring(i, i + 4) == "</b>") {
                            // 退出加粗模式
                            if (boldMode) {
                                val boldOff = "\u001B\u001D\u0045\u0000" // ESC GS E 0 - 加粗关闭
                                Log.d(TAG, "$logPrefix 行${lineIndex+1}: 关闭加粗: ${bytesToHex(boldOff.toByteArray())}")
                                currentPrinter?.printAsync(boldOff)?.await()
                                boldMode = false
                            }
                            i += 4
                        } else {
                            result.append(processedLine[i])
                            i++
                        }
                    }
                    ...
                    if (boldMode) {
                        val boldOff = "\u001B\u001D\u0045\u0000" // ESC GS E 0 - 加粗关闭
                        Log.d(TAG, "$logPrefix 行${lineIndex+1}: 行尾关闭加粗: ${bytesToHex(boldOff.toByteArray())}")
                        currentPrinter?.printAsync(boldOff)?.await()
                        boldMode = false
                    }
                ...
                // 打印当前行
                Log.d(TAG, "$logPrefix 行${lineIndex+1}: 打印内容: ${processedLine.take(30)}${if(processedLine.length > 30) "..." else ""}")
                ...
                if ((lineIndex + 1) % 10 == 0) {
                    try {
                        val lineStatus = currentPrinter?.getStatusAsync()?.await()
                        val statusDesc = when {
                            lineStatus == null -> "状态未知"
                            lineStatus.hasError -> "有错误: ${lineStatus.toString()}"
                            lineStatus.coverOpen -> "纸仓盖打开"
                            lineStatus.paperEmpty -> "缺纸"
                            else -> "正常"
                        }
                        Log.d(TAG, "$logPrefix 打印进度: ${lineIndex+1}/${lines.size}行, 状态: $statusDesc")
                    } catch (e: Exception) {
                        Log.w(TAG, "$logPrefix 获取中间状态失败: ${e.message}")
                    }
            ...
            // 添加走纸和切纸命令
            Log.d(TAG, "$logPrefix 内容打印完成，添加走纸和切纸命令")
            currentPrinter?.printAsync("\n\n\n")?.await()
            delay(200)
            
            val cutCommand = StringBuilder()
                .append("\u001B\u001D\u0065\u0003")     // ESC GS e 3 - 进纸并切纸
                .append("\u001B\u001D\u0056\u0042\u0000") // ESC GS V B 0 - 切纸（带预留）
                .toString()
                
            Log.d(TAG, "$logPrefix 发送切纸命令: ${bytesToHex(cutCommand.toByteArray())}")
            val cutResult = currentPrinter?.printAsync(cutCommand)?.await() != null
            ...
            val finalStatus = try {
                ...
                Log.d(TAG, "$logPrefix 最终打印机状态: $statusDesc")
                status
            } catch (e: Exception) {
                Log.e(TAG, "$logPrefix 获取最终状态失败: ${e.message}")
                null
            }
            ...
            Log.d(TAG, "$logPrefix 光栅模式打印完成，总耗时: ${totalTime}ms")
            Log.d(TAG, "$logPrefix =============== 光栅模式打印结束 ===============")
    ...
    private fun bytesToHex(bytes: ByteArray): String {
        val displayLimit = 50  // 显示的最大字节数
        val displayBytes = bytes.take(displayLimit)
        return displayBytes.joinToString(" ") { String.format("%02X", it.toInt() and 0xFF) } + 
               (if (bytes.size > displayLimit) "... (共${bytes.size}字节)" else "")
    }
