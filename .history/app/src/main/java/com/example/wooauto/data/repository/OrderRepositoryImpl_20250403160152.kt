...
//        Log.d("OrderRepositoryImpl", "更新订单状态: $orderId -> $newStatus")
    ...
    override suspend fun refreshOrders(status: String?, afterDate: Date?): Result<List<Order>> = withContext(Dispatchers.IO) {
        try {
            ...
            Log.d("OrderRepositoryImpl", "找到${readOrderIds.size}个已读订单")
...
//                        Log.d("OrderRepositoryImpl", "【轮询刷新】订单 #${entity.number} 在内存中存在，打印状态: ${cachedOrder.isPrinted}, 已读状态: ${cachedOrder.isRead}")
                        entity.copy(isPrinted = cachedOrder.isPrinted, isRead = cachedOrder.isRead || entity.id in readOrderIds)
                    } else {
                        // 如果内存中不存在但数据库中存在，使用数据库打印状态和已读状态
                        val dbOrder = dbOrderMap[entity.id]
                        if (dbOrder != null) {
//                            Log.d("OrderRepositoryImpl", "【轮询刷新】订单 #${entity.number} 在内存中不存在，使用数据库状态：已打印=${dbOrder.isPrinted}, 已读=${dbOrder.isRead}")
                            entity.copy(
                                isPrinted = dbOrder.isPrinted, 
                                isRead = dbOrder.isRead || entity.id in readOrderIds
                            )
                        } else {
