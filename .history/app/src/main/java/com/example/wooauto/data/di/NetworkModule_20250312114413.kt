...
class NetworkModule {
    ...
    companion object {
        ...
        fun provideOkHttpClient(authInterceptor: WooCommerceAuthInterceptor): OkHttpClient {
            ...
            // 为Android 7及以下版本添加TLS兼容性支持
            if (android.os.Build.VERSION.SDK_INT <= android.os.Build.VERSION_CODES.N) {
                try {
                    Log.d("NetworkModule", "Android 7或更低版本，应用TLS兼容性修复")
                    
                    // 自定义TLSSocketFactory，强制使用TLS 1.2
                    ...
                    builder.sslSocketFactory(tlsSocketFactory, TrustAllCertificatesManager())
                    builder.connectionSpecs(listOf(connectionSpec, okhttp3.ConnectionSpec.COMPATIBLE_TLS, okhttp3.ConnectionSpec.CLEARTEXT))
                    builder.hostnameVerifier { _, _ -> true }
                } catch (e: Exception) {
                    Log.e("NetworkModule", "应用TLS兼容性修复失败: ${e.message}")
                }
            } else {
                // 新版本Android使用默认的SSL/TLS设置即可
                Log.d("NetworkModule", "Android 8或更高版本，使用默认TLS设置")
            }
         ...
         * 仅用于旧版本Android的兼容性处理
         */
        private class TrustAllCertificatesManager : javax.net.ssl.X509TrustManager {
