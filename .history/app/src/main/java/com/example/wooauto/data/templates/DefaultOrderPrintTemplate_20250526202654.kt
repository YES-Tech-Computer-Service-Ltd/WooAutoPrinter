...
class DefaultOrderPrintTemplate @Inject constructor(
    ...
    private fun generate58mmTestContent(config: PrinterConfig): String {
        ...
        sb.append(ThermalPrinterFormatter.formatTitle("58MM 打印测试", paperWidth))
        sb.append(ThermalPrinterFormatter.formatCenteredText("Receipt Printing Test", paperWidth))
        sb.append(ThermalPrinterFormatter.addEmptyLines(1))
        
        // 时间戳
        val dateFormat = SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault())
        sb.append(ThermalPrinterFormatter.formatCenteredText("测试时间：${dateFormat.format(Date())}", paperWidth))
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        
        // 打印机信息 (58mm简化版)
        sb.append(ThermalPrinterFormatter.formatLeftText("打印机信息：", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLabelValue("名称", config.name, paperWidth))
        sb.append(ThermalPrinterFormatter.formatLabelValue("地址", config.address.takeLast(8), paperWidth))
        sb.append(ThermalPrinterFormatter.formatLabelValue("品牌", config.brand.displayName, paperWidth))
        sb.append(ThermalPrinterFormatter.formatLabelValue("纸宽", "58mm (32字符/行)", paperWidth))
        
        // 商店信息测试
        if (storeName.isNotEmpty() || storeAddress.isNotEmpty() || storePhone.isNotEmpty()) {
            sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
            sb.append(ThermalPrinterFormatter.formatLeftText("商店信息测试：", paperWidth))
            if (storeName.isNotEmpty()) {
                sb.append(ThermalPrinterFormatter.formatCenteredText(storeName, paperWidth))
            }
            if (storeAddress.isNotEmpty()) {
            ...
            if (storePhone.isNotEmpty()) {
                sb.append(ThermalPrinterFormatter.formatCenteredText("电话: $storePhone", paperWidth))
            }
        }
        
        // 对齐测试
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("对齐测试：", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("←左对齐文本", paperWidth))
        sb.append(ThermalPrinterFormatter.formatCenteredText("●居中对齐文本●", paperWidth))
        sb.append(ThermalPrinterFormatter.formatRightText("右对齐文本→", paperWidth))
        
        // 58mm商品列表测试 (紧凑格式)
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        sb.append(ThermalPrinterFormatter.formatCenteredText("商品列表测试", paperWidth))
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        
        // 58mm适配的商品格式
        sb.append(ThermalPrinterFormatter.formatLeftText("2x 宫保鸡丁", paperWidth))
        sb.append(ThermalPrinterFormatter.formatRightText("¥38.00", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("1x 麻婆豆腐", paperWidth))
        sb.append(ThermalPrinterFormatter.formatRightText("¥28.00", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("3x 米饭", paperWidth))
        sb.append(ThermalPrinterFormatter.formatRightText("¥9.00", paperWidth))
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        
        // 总计测试
        sb.append(ThermalPrinterFormatter.formatLeftRightText("小计：", "¥75.00", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftRightText("税费：", "¥7.50", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftRightText("总计：", "¥82.50", paperWidth))
        
        // 字符测试
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("字符集测试：", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("中文：测试打印正常", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("英文：Test Print OK", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("数字：0123456789", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("符号：!@#$%^&*()", paperWidth))
        
        // 58mm结尾
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        sb.append(ThermalPrinterFormatter.formatCenteredText("58MM测试完成", paperWidth))
        sb.append(ThermalPrinterFormatter.formatCenteredText("Test Completed", paperWidth))
        
        if (config.autoCut) {
            sb.append(ThermalPrinterFormatter.formatCenteredText("自动切纸已启用", paperWidth))
        } else {
            sb.append(ThermalPrinterFormatter.formatCenteredText("手动撕纸", paperWidth))
        }
    ...
    private fun generate80mmTestContent(config: PrinterConfig): String {
        ...
        sb.append(ThermalPrinterFormatter.formatTitle("80MM 热敏打印机测试", paperWidth))
        ...
        sb.append(ThermalPrinterFormatter.formatCenteredText("测试执行时间：${dateFormat.format(Date())}", paperWidth))
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        
        // 详细打印机信息
        sb.append(ThermalPrinterFormatter.formatCenteredText("打印机配置信息", paperWidth))
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        sb.append(ThermalPrinterFormatter.formatLabelValue("打印机名称", config.name, paperWidth))
        sb.append(ThermalPrinterFormatter.formatLabelValue("蓝牙地址", config.address, paperWidth))
        sb.append(ThermalPrinterFormatter.formatLabelValue("打印机品牌", config.brand.displayName, paperWidth))
        sb.append(ThermalPrinterFormatter.formatLabelValue("纸张宽度", "80mm (42字符/行)", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLabelValue("自动切纸", if (config.autoCut) "已启用" else "已禁用", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLabelValue("自动打印", if (config.isAutoPrint) "已启用" else "已禁用", paperWidth))
        
        // 商店信息完整测试
        if (storeName.isNotEmpty() || storeAddress.isNotEmpty() || storePhone.isNotEmpty()) {
            sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
            sb.append(ThermalPrinterFormatter.formatCenteredText("商店信息显示测试", paperWidth))
            sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
            if (storeName.isNotEmpty()) {
                sb.append(ThermalPrinterFormatter.formatCenteredText(storeName, paperWidth))
            } else {
                sb.append(ThermalPrinterFormatter.formatCenteredText("[请在设置中配置商店名称]", paperWidth))
            }
            if (storeAddress.isNotEmpty()) {
                sb.append(ThermalPrinterFormatter.formatCenteredText(storeAddress, paperWidth))
            } else {
                sb.append(ThermalPrinterFormatter.formatCenteredText("[请在设置中配置商店地址]", paperWidth))
            }
            if (storePhone.isNotEmpty()) {
                sb.append(ThermalPrinterFormatter.formatCenteredText("联系电话: $storePhone", paperWidth))
            } else {
                sb.append(ThermalPrinterFormatter.formatCenteredText("[请在设置中配置联系电话]", paperWidth))
            }
        }
        
        // 对齐和格式测试
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        sb.append(ThermalPrinterFormatter.formatCenteredText("文本对齐和格式测试", paperWidth))
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("←←← 左对齐文本测试 (LEFT ALIGNMENT)", paperWidth))
        sb.append(ThermalPrinterFormatter.formatCenteredText("●●● 居中对齐文本测试 (CENTER ALIGNMENT) ●●●", paperWidth))
        sb.append(ThermalPrinterFormatter.formatRightText("右对齐文本测试 (RIGHT ALIGNMENT) →→→", paperWidth))
        
        // 80mm完整商品列表测试
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        sb.append(ThermalPrinterFormatter.formatCenteredText("完整商品订单格式测试", paperWidth))
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("订单编号: WO2024030601", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("下单时间: ${SimpleDateFormat("yyyy-MM-dd HH:mm", Locale.getDefault()).format(Date())}", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("客户姓名: 张三", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("联系电话: 138****8888", paperWidth))
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        
        // 80mm详细商品列表
        sb.append(ThermalPrinterFormatter.formatLeftRightText("商品名称", "数量 x 单价", paperWidth))
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        sb.append(ThermalPrinterFormatter.formatItemPriceLine("红烧肉套餐", 2, "45.00", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("  配菜选择: 米饭, 青菜, 汤", paperWidth))
        sb.append(ThermalPrinterFormatter.formatItemPriceLine("宫保鸡丁", 1, "32.00", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("  口味要求: 中辣, 少油", paperWidth))
        sb.append(ThermalPrinterFormatter.formatItemPriceLine("酸辣汤", 2, "12.00", paperWidth))
        sb.append(ThermalPrinterFormatter.formatItemPriceLine("可乐 (大杯)", 3, "8.00", paperWidth))
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        
        // 详细费用计算
        sb.append(ThermalPrinterFormatter.formatLeftRightText("商品小计:", "¥146.00", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftRightText("包装费:", "¥3.00", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftRightText("配送费:", "¥5.00", paperWidth))
        ...
        sb.append(ThermalPrinterFormatter.formatLeftRightText("小费:", "¥15.00", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftRightText("优惠折扣:", "-¥10.00", paperWidth))
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftRightText("订单总计:", "¥177.48", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftRightText("支付方式:", "信用卡支付", paperWidth))
        
        // 字符编码和特殊字符测试
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        sb.append(ThermalPrinterFormatter.formatCenteredText("字符编码测试", paperWidth))
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("简体中文: 这是简体中文字符测试", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("繁體中文: 這是繁體中文字符測試", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("English: This is English character test", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("Français: Ceci est un test français", paperWidth))
        ...
        sb.append(ThermalPrinterFormatter.formatLeftText("Currency: ¥ $ € £ ₩ ₹", paperWidth))
        
        // 备注测试
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("订单备注:", paperWidth))
        sb.append(ThermalPrinterFormatter.formatMultilineText("请在指定时间配送，不要打扰邻居。\n如有问题请提前联系，谢谢配合！", paperWidth))
        
        // 80mm结尾信息
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        sb.append(ThermalPrinterFormatter.formatCenteredText("80MM 热敏打印机测试完成", paperWidth))
        sb.append(ThermalPrinterFormatter.formatCenteredText("80MM Thermal Printer Test Completed", paperWidth))
        sb.append(ThermalPrinterFormatter.formatCenteredText("如果以上内容显示正常，说明打印机工作正常", paperWidth))
        
        if (config.autoCut) {
            sb.append(ThermalPrinterFormatter.formatCenteredText("自动切纸功能已启用，将自动切纸", paperWidth))
        } else {
            sb.append(ThermalPrinterFormatter.formatCenteredText("自动切纸功能已禁用，请手动撕纸", paperWidth))
        }
    ...
    private fun generateDefaultTestContent(config: PrinterConfig): String {
        ...
        sb.append(ThermalPrinterFormatter.formatTitle("打印机测试", paperWidth))
        sb.append(ThermalPrinterFormatter.addEmptyLines(1))
        ...
        sb.append(ThermalPrinterFormatter.formatLabelValue("蓝牙地址", config.address, paperWidth))
        sb.append(ThermalPrinterFormatter.formatLabelValue("打印机品牌", config.brand.displayName, paperWidth))
        sb.append(ThermalPrinterFormatter.formatLabelValue("纸张宽度", "${paperWidth}mm", paperWidth))
        
        // 对齐测试
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftText("左对齐测试", paperWidth))
        sb.append(ThermalPrinterFormatter.formatCenteredText("居中对齐测试", paperWidth))
        sb.append(ThermalPrinterFormatter.formatRightText("右对齐测试", paperWidth))
        
        // 简单商品测试
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        sb.append(ThermalPrinterFormatter.formatItemPriceLine("测试商品A", 2, "25.00", paperWidth))
        sb.append(ThermalPrinterFormatter.formatItemPriceLine("测试商品B", 1, "15.00", paperWidth))
        sb.append(ThermalPrinterFormatter.formatLeftRightText("总计:", "¥65.00", paperWidth))
        
        // 结尾
        sb.append(ThermalPrinterFormatter.formatDivider(paperWidth))
        sb.append(ThermalPrinterFormatter.formatCenteredText("测试完成", paperWidth))
        sb.append(ThermalPrinterFormatter.addEmptyLines(3))
