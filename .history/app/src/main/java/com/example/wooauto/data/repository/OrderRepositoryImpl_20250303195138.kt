...
class OrderRepositoryImpl @Inject constructor(
    ...
    override suspend fun refreshOrders(status: String?, afterDate: Date?): Result<List<Order>> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            Log.d("OrderRepositoryImpl", "调用API获取订单列表，参数: $queryParams")
            val response = api.getOrders(1, 100, status)
            Log.d("OrderRepositoryImpl", "成功获取订单列表，共 ${response.size} 个订单")
            ...
            Log.e("OrderRepositoryImpl", "刷新订单数据失败: ${error.code} - ${error.message}", e)
