...
fun PrinterDetailsScreen(
    ...
    // 获取现有打印机配置或创建新的
    val initialPrinterConfig = if (isNewPrinter) {
        PrinterConfig(
    ...
    var printerConfig by remember { mutableStateOf(initialPrinterConfig) }
        ...
        topBar = {
            TopAppBar(
                title = { 
                    Text(
                        if (isNewPrinter) {
                            if (showConfigDialog) stringResource(id = R.string.setup_printer) else stringResource(id = R.string.add_printer)
                        } else stringResource(id = R.string.edit_printer)
                    ) 
                },
                navigationIcon = {
                    IconButton(onClick = { 
                        if (isNewPrinter && showConfigDialog) {
        ...
        if (isNewPrinter && !showConfigDialog) {
            ...
            DeviceListContent(
                ...
                onDeviceSelected = { device ->
                    ...
                    printerConfig = printerConfig.copy(
                        name = device.name,
            ...
            ConfigurationScreen(
                ...
                onSave = {
                    // 验证必填字段
                    if (printerConfig.name.isBlank() || printerConfig.address.isBlank()) {
            ...
            if (isScanning) {
                ...
                Text(
                    text = if (availablePrinters.isEmpty()) 
                            ...
                            else stringResource(id = R.string.bluetooth_devices),
...
private fun DeviceItem(
    ...
    val isPaired = remember(device.address) {
        try {
            ...
            if (bluetoothAdapter != null) {
                ...
                bluetoothAdapter.bondedDevices?.any { it.address == device.address } ?: false
            } else {
                false
            }
        } catch (e: Exception) {
    ...
    // 提前获取字符串资源
    val pairedText = stringResource(id = R.string.paired)
    val selectText = stringResource(id = R.string.select)
...
private fun RequestPermissionButton() {
    val context = LocalContext.current
    val requestPermissionText = stringResource(id = R.string.request_bluetooth_location_permission)
    ...
    Button(
        ...
        Text(text = requestPermissionText)
    }
} 