        ...
        try {
            ...
            val orders = response.map { orderDto -> 
                ...
                if (order.id in readOrderIds) {
                    ...
                    if (existingOrder != null && existingOrder.isRead) {
                        order.copy(isRead = true)
                    } else {
                        order
                    }
            ...
            val orderEntities = orders.map { order ->
                ...
                if (existingEntity != null) {
                    // 保留已读状态：优先使用即将保存的实体状态，如果没有变化则使用现有状态
                    val isRead = if (entity.isRead || existingEntity.isRead) true else false
                    
                    // 日志记录状态变化
                    if (isRead != entity.isRead) {
                        Log.d("OrderRepositoryImpl", "订单 #${entity.number} (ID=${entity.id}) 已读状态从 ${entity.isRead} 更新为 $isRead")
                    }
                    
                    // 保留打印状态
                    val isPrinted = existingEntity.isPrinted
