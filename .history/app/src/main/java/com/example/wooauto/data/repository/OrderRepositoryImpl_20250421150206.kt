...
//        Log.d("OrderRepositoryImpl", "刷新特定状态订单: $status")
     ...
     * @return 订单列表
     */
    override suspend fun getOrdersByStatus(status: String): List<Order> = withContext(Dispatchers.IO) {
        try {
            Log.d("OrderRepositoryImpl", "【状态筛选】从数据库加载状态为 '$status' 的订单")
            val orderEntities = orderDao.getOrdersByStatus(status)
            Log.d("OrderRepositoryImpl", "【状态筛选】从数据库获取到 ${orderEntities.size} 个状态为 '$status' 的订单")
            
            // 将数据库实体转换为领域模型，保留打印状态
            val orders = orderEntities.map { mapToOrderModel(it) }
            
            // 打印状态统计
            val printedCount = orders.count { it.isPrinted }
            Log.d("OrderRepositoryImpl", "【状态筛选】已打印订单: $printedCount / ${orders.size}")
            
            return@withContext orders
        } catch (e: Exception) {
            Log.e("OrderRepositoryImpl", "【状态筛选】获取订单列表失败", e)
            return@withContext emptyList()
        }
    }
}