...
class DefaultOrderPrintTemplate @Inject constructor(
    ...
    private fun generatePaymentInfo(order: Order, config: PrinterConfig, templateConfig: TemplateConfig,
        ...
        val sb = StringBuilder()
        val left = if (templateConfig.stylePaymentMethodBold) ThermalPrinterFormatter.formatBold("payment method:") else "payment method:"
        val right = if (templateConfig.stylePaymentMethodLarge || templateConfig.stylePaymentMethodBold) {
            buildString {
                if (templateConfig.stylePaymentMethodLarge) append("<h><w>")
                if (templateConfig.stylePaymentMethodBold) append("<b>")
                append(order.paymentMethod)
                if (templateConfig.stylePaymentMethodBold) append("</b>")
                if (templateConfig.stylePaymentMethodLarge) append("</w></h>")
            }
