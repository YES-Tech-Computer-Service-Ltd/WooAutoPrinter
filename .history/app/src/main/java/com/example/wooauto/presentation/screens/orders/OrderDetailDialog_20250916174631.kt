 ...
 * 订单详情对话框
 */
// 模板模式：AUTO 按订单状态/已读自动判断；NEW 与 PROCESSING 为显式模式
        ...
        Card(
            ...
            Column(
                ...
                Row(
                    ...
                    // 中间按钮：根据 isRead 与 status 区分
                    when {
                        // New orders：未读，即便状态是 processing，也显示“开始处理”
                        !displayOrder.isRead -> {
                            OutlinedButton(
                                onClick = {
                                    // 标记为已读；如非 processing 则置为 processing
                        ...
                        // In processing：已读且状态为 processing，显示“标记为已完成”
