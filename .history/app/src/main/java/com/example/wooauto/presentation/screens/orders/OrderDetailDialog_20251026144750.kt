            ...
            Column(
                ...
                TopAppBar(
                    ...
                    actions = {
                        ...
                        val status = remember(displayOrder.id, scheduledDateStr, timeText) {
                ...
                Box(
                    ...
                    Column(
                        ...
                        displayOrder.woofoodInfo?.let { wooFoodInfo ->
                            ...
                            // 统一显示预计时间（配送/自取）——若能解析到日期则一起显示
                            wooFoodInfo.deliveryTime?.let { time ->
                                val scheduledDate = extractScheduledDateFromNotes(displayOrder.notes)
                                val dateTimeText = if (!scheduledDate.isNullOrBlank()) "$scheduledDate $time" else time
                                OrderDetailRow(
...
private fun extractScheduledDateFromNotes(note: String): String? {
    ...
    return try {
        ...
        for (key in keys) {
            val pattern = ("" + key + "[：:]*\\s*([\\\\w\\\\-/. ,]+)").toRegex(RegexOption.IGNORE_CASE)
...
private fun buildOrderStatusLabel(
    ...
    val relative = computeRelativeDay(createdAt, dateOnly)
    ...
    val datePortion = if (relative != null) "$dateStr（$relative）" else dateStr
...
private fun computeRelativeDay(createdAt: java.util.Date, scheduled: java.util.Date): String? {
    ...
    return when (days) {
        ...
        1 -> "明天"
        2 -> "后天"
        else -> if (days > 2) "${days}天后" else null
    }
}
