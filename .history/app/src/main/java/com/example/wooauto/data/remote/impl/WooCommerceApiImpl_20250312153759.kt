...
class WooCommerceApiImpl(
    ...
    private fun createSimpleClient(): OkHttpClient {
        try {
            ...
            // 使用这个TrustManager创建一个SSLContext - 注意：使用TLSv1而不是具体版本
            val sslContext = SSLContext.getInstance("TLS")
            sslContext.init(null, trustAllCerts, java.security.SecureRandom())
            
            // 创建一个自定义SSLSocketFactory，它会尝试启用所有可用的密码套件
            val sslSocketFactory = object : SSLSocketFactory() {
                ...
                override fun createSocket(s: java.net.Socket, host: String, port: Int, autoClose: Boolean): java.net.Socket {
                    ...
                    try {
                        Log.d("WooCommerceApiImpl", "配置SSL套接字参数")
                        
                        // 输出可用的协议和密码套件，帮助调试
                        Log.d("WooCommerceApiImpl", "支持的协议: ${socket.supportedProtocols.joinToString()}")
                        Log.d("WooCommerceApiImpl", "支持的密码套件: ${socket.supportedCipherSuites.joinToString()}")
                        ...
                        Log.d("WooCommerceApiImpl", "已启用所有协议和密码套件")
                    } catch (e: Exception) {
                        Log.w("WooCommerceApiImpl", "配置SSL套接字失败: ${e.message}")
                    }
                    return socket
                }
            ...
            // 创建连接规范 - 使用COMPATIBLE_TLS (这比MODERN_TLS兼容性更好)
            val legacySpec = ConnectionSpec.Builder(ConnectionSpec.COMPATIBLE_TLS)
                .tlsVersions(TlsVersion.TLS_1_0)  // 只使用TLS 1.0，更兼容Android 7
                .cipherSuites(
                    // 列出所有支持的密码套件，特别关注老旧的TLS兼容的套件
            ...
            val dnsInterceptor = Interceptor { chain ->
                ...
                Log.d("WooCommerceApiImpl", "准备连接到主机: $host")
                
                try {
                    chain.proceed(originalRequest)
                } catch (e: Exception) {
                    Log.e("WooCommerceApiImpl", "连接失败: ${e.message}")
                    throw e
                }
            }
            
            // 创建OkHttpClient - 注意：只使用HTTP/1.1协议
            return OkHttpClient.Builder()
                .sslSocketFactory(sslSocketFactory, trustAllCerts[0] as X509TrustManager)
                .hostnameVerifier { hostname, session -> 
                    Log.d("WooCommerceApiImpl", "验证主机名: $hostname")
                    true  // 禁用主机名验证
                }
                .connectionSpecs(listOf(legacySpec, clearTextSpec))
                .protocols(listOf(Protocol.HTTP_1_1))  // 只使用HTTP/1.1，移除HTTP/1.0
            ...
            // 基本客户端只使用HTTP/1.1
            return OkHttpClient.Builder()
                .protocols(listOf(Protocol.HTTP_1_1))  // 只使用HTTP/1.1
                .readTimeout(30, TimeUnit.SECONDS)
