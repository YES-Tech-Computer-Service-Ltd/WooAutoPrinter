...
class OrderRepositoryImpl @Inject constructor(
    ...
    override fun getOrdersByStatusFlow(status: String): Flow<List<Order>> {
        return _ordersByStatusFlow.asStateFlow().let { flow ->
            kotlinx.coroutines.flow.map(flow) { map ->
                map[status] ?: emptyList()
            }
        }
    }
    ...
    override suspend fun markOrderAsPrinted(orderId: Long): Result<Unit> {
        return try {
            ...
            if (success) Result.success(Unit) else Result.failure(Exception("Failed to mark order as printed"))
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
