...
fun WooAppBar(
    ...
    val templateIdArg = navBackStackEntry?.arguments?.getString("templateId")
    ...
    // 模板预览/编辑页：动态标题
    val templatePreviewTitle: String? = run {
        if (currentRoute.startsWith("template_preview/")) {
            val resId = when (templateIdArg) {
    ...
    when (currentRoute) {
        NavigationItem.Orders.route -> {
            ...
            WooTopBar(
                title = templatePreviewTitle ?: compositeTitle ?: stringResource(id = spec.titleResId),
                showSearch = false,
                isRefreshing = false,
                onRefresh = { },
