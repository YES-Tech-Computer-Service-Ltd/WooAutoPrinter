...
class BackgroundPollingService : Service() {
    ...
    override fun onDestroy() {
        Log.d(TAG, "BackgroundPollingService正在销毁")
        ...
        try {
            ...
            // 注销广播接收器
            try {
                LocalBroadcastManager.getInstance(this).unregisterReceiver(foregroundStateReceiver)
            } catch (e: Exception) {
            ...
            Log.d(TAG, "BackgroundPollingService已销毁")
        } catch (e: Exception) {
            Log.e(TAG, "销毁服务时出错: ${e.message}", e)
        }
    ...
    private fun startPeriodicCleanupTask() {
        ...
        // 等待取消完成
        while (cleanupJob?.isActive == true) {
            Thread.sleep(100)
        }
            ...
            try {
                while (isActive) {
                    ...
                    try {
                        ...
                        Log.e(TAG, "清理操作异常: ${e.message}")
                        // 继续循环，不要因为单次清理失败就退出
                    }
                }
            } catch (e: CancellationException) {
                // 协程被取消是正常情况
                Log.d(TAG, "定期清理任务已正常取消")
                throw e // 重新抛出以确保协程正确结束
            } catch (e: Exception) {
                Log.e(TAG, "定期清理任务发生未预期异常: ${e.message}", e)
            } finally {
                Log.d(TAG, "定期清理任务已结束")
            }
        }
