...
data class Order(
    ...
    val shippingAddress: String = "", // 配送地址，在WooFood模式下如果为空则使用billingInfo
    ...
    /**
     * 获取实际的配送地址
     * 优先级：shippingAddress -> woofoodInfo.deliveryAddress -> billingInfo
     */
    fun getEffectiveShippingAddress(): String {
        return when {
            shippingAddress.isNotBlank() && shippingAddress != billingInfo -> shippingAddress
            woofoodInfo?.deliveryAddress?.isNotBlank() == true -> woofoodInfo.deliveryAddress
            else -> billingInfo
        }
    }
}
