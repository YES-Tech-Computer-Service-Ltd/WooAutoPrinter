...
fun WooAppBar(
    ...
    // 模板预览/编辑页：显示 “Printing - {模板名}”
    val templateCompositeTitle: String? = run {
        if (currentRoute.startsWith("template_preview/")) {
            val templateId = navBackStackEntry?.arguments?.getString("templateId")
            ...
            stringResource(R.string.printing) + " - " + stringResource(nameRes)
        } else null
    }
    when (currentRoute) {
        NavigationItem.Orders.route -> {
            ...
            WooTopBar(
                title = templateCompositeTitle ?: compositeTitle ?: stringResource(id = spec.titleResId),
                showSearch = false,
                isRefreshing = false,
                onRefresh = { },
