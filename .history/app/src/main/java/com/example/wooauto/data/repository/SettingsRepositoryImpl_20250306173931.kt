...
class SettingsRepositoryImpl @Inject constructor(
    ...
    init {
        ...
        GlobalScope.launch {
            try {
                ...
                Log.d("SettingsRepositoryImpl", "初始化检查配置有效性: $isValid")
                WooCommerceConfig.updateConfigurationStatus(isValid)
            } catch (e: Exception) {
    ...
    override suspend fun saveWooCommerceConfig(config: WooCommerceConfig) {
        ...
        Log.d("SettingsRepositoryImpl", "配置有效性检查: $isConfigValid")
        WooCommerceConfig.updateConfigurationStatus(isConfigValid)
    }
    ...
    override suspend fun getDefaultTemplateType(): TemplateType? {
        val key = "default_template_type"
        val entity = settingDao.getSettingByKey(key)
        return if (entity == null) {
            TemplateType.FULL_DETAILS // 默认返回完整模板
        } else {
            try {
                TemplateType.valueOf(entity.value)
            } catch (e: Exception) {
                Log.e("SettingsRepository", "解析默认模板类型失败: ${e.message}")
                TemplateType.FULL_DETAILS // 出错时返回默认模板
            }
        }
    }
    ...
    override suspend fun saveDefaultTemplateType(templateType: TemplateType) {
        ...
        settingDao.insertOrUpdateSetting(entity)
        Log.d("SettingsRepository", "保存默认模板类型: $templateType")
    }
} 