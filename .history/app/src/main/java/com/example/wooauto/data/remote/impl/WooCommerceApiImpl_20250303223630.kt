                    ...
                    try {
                        ...
                        when (result) {
                            is List<*> -> {
                                ...
                                val className = if (result != null) result::class.java.simpleName else "null"
                                Log.d("WooCommerceApiImpl", "【订单调试】成功解析单个对象: $className")
                            }
                        }
                        ...
                        if (endpoint.contains("orders") && T::class.java == List::class.java) {
                            Log.w("WooCommerceApiImpl", "【订单调试】尝试使用备用解析方法")
                            try {
                                // 尝试手动解析JSON提取订单数据
                                val manuallyExtractedOrders = extractOrdersManually(responseBody)
                                if (manuallyExtractedOrders.isNotEmpty()) {
                                    Log.d("WooCommerceApiImpl", "【订单调试】手动解析成功，提取了 ${manuallyExtractedOrders.size} 个订单")
                                    @Suppress("UNCHECKED_CAST")
                                    return@withContext manuallyExtractedOrders as T
                                } else {
                                    Log.w("WooCommerceApiImpl", "【订单调试】手动解析失败，返回空列表")
                                    @Suppress("UNCHECKED_CAST")
                                    return@withContext emptyList<OrderDto>() as T
                                }
                            } catch (e: Exception) {
                                Log.e("WooCommerceApiImpl", "【订单调试】备用解析方法失败: ${e.message}")
                                @Suppress("UNCHECKED_CAST")
                                return@withContext emptyList<OrderDto>() as T
                            }
                        }
