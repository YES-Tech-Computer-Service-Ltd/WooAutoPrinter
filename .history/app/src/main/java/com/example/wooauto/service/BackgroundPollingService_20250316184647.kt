...
class BackgroundPollingService : Service() {
    ...
    private suspend fun pollOrders() {
        try {
            ...
            // 执行轮询
            val result = orderRepository.refreshProcessingOrders(false)
    ...
    private suspend fun checkPrinterConnection() {
        try {
            ...
            printerConfig?.let { config ->
                val status = printerManager.getPrinterStatus(config)
                
                if (status != PrinterStatus.CONNECTED) {
                    Log.d(TAG, "轮询检测到打印机未连接，尝试重新连接: ${config.name}")
                    
                    // 尝试连接打印机
                    val connected = printerManager.connect(config)
                    
                    Log.d(TAG, "打印机连接结果: $connected (${config.name})")
                } else {
                    Log.d(TAG, "打印机已连接: ${config.name}")
                }
            }
        } catch (e: Exception) {
            Log.e(TAG, "检查打印机连接状态时出错: ${e.message}", e)
        }
    }
