...
class StarPrinterHandler(private val context: Context) {
    ...
    suspend fun printWithRasterMode(content: String): Boolean {
        ...
        try {
            val isTSP100Series = currentModel?.contains("TSP100") ?: false
            Log.d(TAG, "$logPrefix 开始处理打印内容，长度: ${content.length}字符")
            ...
            Log.d(TAG, "$logPrefix 初始打印机状态: $initialStatus")
            ...
            if (lines.size > 10) {
                Log.d(TAG, "$logPrefix 更多行数未显示...")
            }
            
            // 初始化打印机
            val initCommand = ESC_POS_INIT
            Log.d(TAG, "$logPrefix 发送初始化命令: ${bytesToHex(initCommand.toByteArray())}")
            appendBytes(initCommand.toByteArray())
            
            // 进入光栅模式
            val startRasterCmd = if (isTSP100Series) TSP100_START_RASTER else ESC_POS_START_RASTER
            Log.d(TAG, "$logPrefix 发送光栅模式命令: ${bytesToHex(startRasterCmd.toByteArray())}")
            appendBytes(startRasterCmd.toByteArray())
            
            // 暂存所有处理后的行数据
            val processedLines = mutableListOf<ByteArray>()
            var totalBytes = 0
            var currentAlignment = ALIGN_LEFT
            ...
            lines.forEachIndexed { index, line ->
                ...
                if (trimmedLine.startsWith("[L]")) {
                    if (currentAlignment != ALIGN_LEFT) {
                        Log.d(TAG, "$logPrefix 行${index+1}: 切换到左对齐")
                        currentAlignment = ALIGN_LEFT
                        appendBytes(currentAlignment.toByteArray())
                    }
                    // 处理去掉标签的内容
                    val processedLine = processRasterLine(trimmedLine.substring(3))
                    Log.d(TAG, "$logPrefix 行${index+1}(左对齐): ${processedLine.take(30)}${if(processedLine.length > 30) "..." else ""}")
                    processedLines.add(processedLine.toByteArray())
                    totalBytes += processedLine.toByteArray().size
                } else if (trimmedLine.startsWith("[C]")) {
                    if (currentAlignment != ALIGN_CENTER) {
                        Log.d(TAG, "$logPrefix 行${index+1}: 切换到居中对齐")
                        currentAlignment = ALIGN_CENTER
                        appendBytes(currentAlignment.toByteArray())
                    }
                    // 处理去掉标签的内容
                    val processedLine = processRasterLine(trimmedLine.substring(3))
                    Log.d(TAG, "$logPrefix 行${index+1}(居中对齐): ${processedLine.take(30)}${if(processedLine.length > 30) "..." else ""}")
                    processedLines.add(processedLine.toByteArray())
                    totalBytes += processedLine.toByteArray().size
                } else if (trimmedLine.startsWith("[R]")) {
                    if (currentAlignment != ALIGN_RIGHT) {
                        Log.d(TAG, "$logPrefix 行${index+1}: 切换到右对齐")
                        currentAlignment = ALIGN_RIGHT
                        appendBytes(currentAlignment.toByteArray())
                    }
                    // 处理去掉标签的内容
                    val processedLine = processRasterLine(trimmedLine.substring(3))
                    Log.d(TAG, "$logPrefix 行${index+1}(右对齐): ${processedLine.take(30)}${if(processedLine.length > 30) "..." else ""}")
                    processedLines.add(processedLine.toByteArray())
                    totalBytes += processedLine.toByteArray().size
                } else {
                    // 无标签，使用当前对齐方式
                    Log.d(TAG, "$logPrefix 行${index+1}(保持当前对齐): 处理原始内容")
                    val processedLine = processRasterLine(trimmedLine)
                    Log.d(TAG, "$logPrefix 行${index+1}: ${processedLine.take(30)}${if(processedLine.length > 30) "..." else ""}")
                    processedLines.add(processedLine.toByteArray())
                    totalBytes += processedLine.toByteArray().size
                }
            ...
            Log.d(TAG, "$logPrefix 所有行处理完成，总计 ${processedLines.size} 个数据块，${totalBytes} 字节")
            
            // 设计分段大小
            val chunkSize = 1024
            val totalChunks = (totalBytes + chunkSize - 1) / chunkSize
            
            Log.d(TAG, "$logPrefix 分段打印：总计 $totalChunks 段，每段最大 $chunkSize 字节")
            ...
            // 合并所有处理过的行
            for (lineBytes in processedLines) {
                // 检查是否会溢出当前块
                if (sentBytes + lineBytes.size > currentChunk * chunkSize) {
                    // 需要打印当前缓冲区并重置
                    Log.d(TAG, "$logPrefix 发送分段 $currentChunk/$totalChunks，已发送 $sentBytes 字节")
                    val printResult = printBuffer()
                    if (!printResult) {
                        Log.e(TAG, "$logPrefix 打印分段 $currentChunk 失败")
                        bufferOverflow = true
                        break
                    }
                    ...
                    // 每5个段检查一次打印机状态
                    if (currentChunk % 5 == 0) {
                        val statusInfo = checkStatus()
                        Log.d(TAG, "$logPrefix 分段 $currentChunk 打印后状态检查: $statusInfo")
                    }
                ...
                // 添加当前行的字节
                appendBytes(lineBytes)
                sentBytes += lineBytes.size
            }
            
            // 如果处理中断，返回失败
            if (bufferOverflow) {
                Log.e(TAG, "$logPrefix 打印过程中缓冲区溢出，打印失败")
                return false
            }
            
            // 打印剩余内容
            if (sentBytes > 0) {
                Log.d(TAG, "$logPrefix 发送最后一个分段 $currentChunk/$totalChunks，最终总字节数 $sentBytes")
                val finalPrintResult = printBuffer()
                if (!finalPrintResult) {
                    Log.e(TAG, "$logPrefix 打印最后分段失败")
                    return false
                }
            }
            
            // 添加走纸和切纸命令
            Log.d(TAG, "$logPrefix 添加走纸和切纸命令")
            appendBytes("\n\n\n\n".toByteArray())
            appendBytes(CUT_PAPER.toByteArray())
            val cutResult = printBuffer()
            
            // 最终状态检查
            val finalStatus = checkStatus()
            Log.d(TAG, "$logPrefix 打印完成，最终打印机状态: $finalStatus")
            
            Log.d(TAG, "$logPrefix 光栅模式打印完成")
            return cutResult
        } catch (e: Exception) {
            Log.e(TAG, "光栅模式打印异常: ${e.message}")
            e.printStackTrace()
            return false
        }
    }
    ...
    private fun processRasterLine(line: String): String {
        val logPrefix = "【光栅行处理】"
        try {
            Log.d(TAG, "$logPrefix 处理行: ${line.take(30)}${if(line.length > 30) "..." else ""}")
            
            // 处理粗体标签
            var processedLine = line
            if (processedLine.contains("[B]") && processedLine.contains("[/B]")) {
                Log.d(TAG, "$logPrefix 检测到粗体标签")
                // 提取粗体内容
                val boldPattern = "\\[B\\](.*?)\\[/B\\]".toRegex()
                val boldMatches = boldPattern.findAll(processedLine)
                
                // 替换所有粗体标签
                for (match in boldMatches) {
                    val fullMatch = match.value
                    val boldContent = match.groupValues[1]
                    Log.d(TAG, "$logPrefix 粗体内容: $boldContent")
                    
                    // 替换为ESC/POS粗体命令
                    val replacement = "$ESC_BOLD_ON$boldContent$ESC_BOLD_OFF"
                    processedLine = processedLine.replace(fullMatch, replacement)
                }
                
                Log.d(TAG, "$logPrefix 处理粗体后: ${processedLine.take(30)}${if(processedLine.length > 30) "..." else ""}")
            }
            
            // 检查是否还有未处理的格式标签
            val remainingTags = "\\[.*?\\]".toRegex().findAll(processedLine).map { it.value }.toList()
            if (remainingTags.isNotEmpty()) {
                Log.w(TAG, "$logPrefix 警告：存在未处理的格式标签: ${remainingTags.joinToString(", ")}")
            }
            
            return processedLine
        } catch (e: Exception) {
            Log.e(TAG, "$logPrefix 处理行异常: ${e.message}")
            // 出错时返回原行
            return line
        }
    }
    ...
    private fun bytesToHex(bytes: ByteArray): String {
        return bytes.take(20).joinToString(" ") { byte -> String.format("%02X", byte.toInt() and 0xFF) } + 
               (if (bytes.size > 20) "..." else "")
    }
