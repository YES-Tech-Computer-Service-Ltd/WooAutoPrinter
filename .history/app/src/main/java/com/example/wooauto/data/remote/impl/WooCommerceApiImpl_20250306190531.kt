        ...
        while (currentRetry < maxRetries) {
            try {
                return withContext(Dispatchers.IO) {
                    try {
                        // 如果不是第一次尝试，添加延迟（使用指数退避）
                        if (currentRetry > 0) {
                            val delayMs = (2.0.pow(currentRetry.toDouble()).toLong() * 1000)  // 指数退避: 2^retry * 1000ms
                            Log.d("WooCommerceApiImpl", "重试请求(${currentRetry}/$maxRetries)，延迟 $delayMs ms")
                            delay(delayMs)
                        }
                ...
                Log.e("WooCommerceApiImpl", "SSL连接错误，尝试重试 (${currentRetry+1}/$maxRetries): ${e.message}")
                lastException = e
                currentRetry++
            } catch (e: java.net.ConnectException) {
                Log.e("WooCommerceApiImpl", "连接错误，尝试重试 (${currentRetry+1}/$maxRetries): ${e.message}")
                lastException = e
                currentRetry++
