...
class ProfessionalCrashReporter {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_bulk_resolve_reports', array($this, 'ajax_bulk_resolve'));
        add_action('wp_ajax_manual_cleanup_reports', array($this, 'ajax_manual_cleanup'));
        add_action('wp_ajax_get_crash_details', array($this, 'ajax_get_crash_details'));
    ...
    private function create_table() {
        ...
        $sql = "CREATE TABLE $this->table_name (
            ...
            context_logs longtext DEFAULT NULL,
            user_actions text DEFAULT NULL,
            app_state_json text DEFAULT NULL,
    ...
    public function handle_crash_report($request) {
        ...
        if ($file_path) {
            $json_content = file_get_contents($this->upload_dir . $file_path);
            if ($json_content) {
                $crash_json = json_decode($json_content, true);
                if ($crash_json) {
                    $context_logs = sanitize_textarea_field($crash_json['context_logs'] ?? '');
                    $user_actions = sanitize_textarea_field($crash_json['user_actions'] ?? '');
                    $app_state_json = wp_json_encode($crash_json['app_state'] ?? array());
                    $performance_data = sanitize_textarea_field($crash_json['performance_data'] ?? '');
                }
            }
        }
    ...
    public function ajax_bulk_resolve() {
        ...
        $updated_count = 0;
        foreach ($report_ids as $report_id) {
            if ($wpdb->update($this->table_name, array('is_resolved' => 1), array('id' => $report_id))) {
                $updated_count++;
            }
        }
    ...
    public function ajax_manual_cleanup() {
        ...
        $deleted_count = $this->cleanup_old_reports();
    ...
    private function generate_reports_template($reports, $app_versions, $error_types, $total_items, $page, $per_page) {
        ...
        <div class="wrap">
            ...
            <!-- 批量操作 -->
            <div class="bulk-actions-bar" style="margin: 10px 0;">
                <button type="button" id="bulk-delete" class="button button-secondary">批量删除</button>
                <button type="button" id="bulk-resolve" class="button button-secondary">标记已解决</button>
                <span id="bulk-actions-status"></span>
            </div>
        ...
        <div class="wrap">
            ...
            <form method="post" action="">
                ...
                <div class="database-status">
                    <?php
                    global $wpdb;
                    $total_reports = $wpdb->get_var("SELECT COUNT(*) FROM $this->table_name");
                    $old_reports = $wpdb->get_var($wpdb->prepare(
                        "SELECT COUNT(*) FROM $this->table_name WHERE created_at < DATE_SUB(NOW(), INTERVAL %d DAY)",
                        $cleanup_days
                    ));
                    $file_count = 0;
                    $upload_files = glob($this->upload_dir . '*.json');
                    if ($upload_files) {
                        $file_count = count($upload_files);
                    }
                    ?>
                    <table class="widefat">
                        <tr><td>总报告数量</td><td><?php echo number_format($total_reports); ?></td></tr>
                        <tr><td>可清理的旧报告</td><td><?php echo number_format($old_reports); ?></td></tr>
                        <tr><td>上传文件数量</td><td><?php echo number_format($file_count); ?></td></tr>
                    </table>
                    
                    <p>
                        <button type="button" id="manual-cleanup" class="button button-secondary">立即清理旧数据</button>
    ...
    private function get_admin_css() {
        ...
        .filter-row select, .filter-row input[type='text'], .filter-row input[type='date'] { 
            min-width: 150px; padding: 5px; border-radius: 3px; border: 1px solid #ddd;
        }
        .bulk-actions-bar {
        ...
        .severity-critical { background: #dc3545; animation: pulse 2s infinite; }
        @keyframes pulse {
            0% { opacity: 1; }
            50% { opacity: 0.7; }
            100% { opacity: 1; }
        ...
        .page-number:hover { background: #f5f5f5; }
        ...
        .database-status table td {
            padding: 8px; border-bottom: 1px solid #eee;
        }
        .database-status table td:first-child {
            font-weight: bold; width: 200px;
        }
        .crash-reports-table th {
            background: #f1f1f1; font-weight: bold;
        }
        .crash-reports-table tr:hover {
            background: #f9f9f9;
        }
        #bulk-actions-status {
            color: #0073aa; font-weight: bold; margin-left: 10px;
        }
        #cleanup-status {
            color: #0073aa; font-weight: bold; margin-left: 10px;
        }
        ...
        .tab-button {
            ...
            cursor: pointer; margin-right: 5px; border-bottom: 2px solid transparent;
            transition: all 0.2s;
        }
        .tab-button.active {
            border-bottom-color: #0073aa; color: #0073aa; font-weight: bold;
        }
        .tab-button:hover {
            background: #f5f5f5;
        }
        ...
        .crash-detail-table td:first-child {
            font-weight: bold; width: 150px; background: #f9f9f9;
        }
        .crash-stack-trace, .crash-context-logs, .crash-user-actions, .crash-performance-data {
            background: #f4f4f4; padding: 15px; border-radius: 4px; 
            max-height: 400px; overflow-y: auto; white-space: pre-wrap;
            font-family: 'Courier New', monospace; font-size: 12px; line-height: 1.4;
        }
        .crash-context-logs {
            background: #f9f9f9; border-left: 4px solid #0073aa;
        }
        .crash-user-actions {
            background: #f0f8f0; border-left: 4px solid #28a745;
        }
        .crash-performance-data {
            background: #fff8f0; border-left: 4px solid #ffc107;
        }
        ";
    }
    ...
    private function get_admin_js() {
        ...
        jQuery(document).ready(function($) {
            ...
            // 手动清理旧数据
            $('#manual-cleanup').click(function() {
                if (confirm('确定要清理旧数据吗？此操作不可撤销。')) {
                    var button = $(this);
                    button.prop('disabled', true).text('清理中...');
                    
                    $.post(ajaxurl, {
                        action: 'manual_cleanup_reports'
                    }, function(response) {
                        if (response.success) {
                            $('#cleanup-status').text('已清理 ' + response.data.deleted_count + ' 个报告');
                            setTimeout(function() {
                                location.reload();
                            }, 2000);
                        } else {
                        ...
                        button.prop('disabled', false).text('立即清理旧数据');
                    });
                }
            });
            ...
            $('#bulk-delete').click(function() {
                ...
                if (confirm('确定要删除选中的 ' + selectedIds.length + ' 个报告吗？')) {
                    $.post(ajaxurl, {
                        ...
                        if (response.success) {
                            $('.report-checkbox:checked').closest('tr').fadeOut();
                            $('#bulk-actions-status').text('已删除 ' + response.data.deleted_count + ' 个报告');
                            setTimeout(function() {
                                location.reload();
                            }, 1500);
                        } else {
            ...
            $('#bulk-resolve').click(function() {
                ...
                $.post(ajaxurl, {
                    ...
                    if (response.success) {
                        $('.report-checkbox:checked').closest('tr').find('.status-badge')
                            .removeClass('unresolved').addClass('resolved').text('已解决');
                        $('#bulk-actions-status').text('已标记 ' + selectedIds.length + ' 个报告为已解决');
                    } else {
                        alert('批量标记失败');
                    }
                });
            ...
            $('.view-details').click(function() {
                ...
                // 通过AJAX获取详细信息
                $.post(ajaxurl, {
                    action: 'get_crash_details',
                    report_id: reportId
                }, function(response) {
                    if (response.success) {
                        var data = response.data;
                        var content = '<h3>崩溃报告详情 #' + reportId + '</h3>';
                        
                        content += '<div class="crash-detail-tabs">';
                        content += '<button class="tab-button active" data-tab="basic">基本信息</button>';
                        content += '<button class="tab-button" data-tab="stack">堆栈跟踪</button>';
                        content += '<button class="tab-button" data-tab="context">上下文日志</button>';
                        content += '<button class="tab-button" data-tab="user-actions">用户操作</button>';
                        content += '<button class="tab-button" data-tab="performance">性能数据</button>';
                        content += '</div>';
                        
                        // 基本信息标签
                        content += '<div id="tab-basic" class="tab-content active">';
                        content += '<table class="crash-detail-table">';
                        content += '<tr><td>错误类型</td><td>' + data.error_type + '</td></tr>';
                        content += '<tr><td>错误消息</td><td>' + data.error_message + '</td></tr>';
                        content += '<tr><td>应用版本</td><td>' + data.app_version + '</td></tr>';
                        content += '<tr><td>Android版本</td><td>' + data.android_version + '</td></tr>';
                        content += '<tr><td>设备型号</td><td>' + data.device_model + '</td></tr>';
                        content += '<tr><td>发生时间</td><td>' + data.created_at + '</td></tr>';
                        content += '<tr><td>严重程度</td><td><span class="severity-badge severity-' + data.severity + '">' + data.severity + '</span></td></tr>';
                        if (data.app_state) {
                            var appState = JSON.parse(data.app_state);
                            content += '<tr><td>当前屏幕</td><td>' + (appState.current_screen || 'Unknown') + '</td></tr>';
                            content += '<tr><td>应用状态</td><td>' + (appState.app_state || 'Unknown') + '</td></tr>';
                        }
                        content += '</table>';
                        content += '</div>';
                        
                        // 堆栈跟踪标签
                        content += '<div id="tab-stack" class="tab-content">';
                        content += '<pre class="crash-stack-trace">' + data.stack_trace + '</pre>';
                        content += '</div>';
                        
                        // 上下文日志标签
                        content += '<div id="tab-context" class="tab-content">';
                        if (data.context_logs) {
                            content += '<pre class="crash-context-logs">' + data.context_logs + '</pre>';
                        } else {
                            content += '<p>无上下文日志</p>';
                        }
                        content += '</div>';
                        ...
                        // 用户操作标签
                        content += '<div id="tab-user-actions" class="tab-content">';
                        if (data.user_actions) {
                            content += '<pre class="crash-user-actions">' + data.user_actions + '</pre>';
                        } else {
                            content += '<p>无用户操作记录</p>';
                        }
                        content += '</div>';
                        ...
                        // 性能数据标签
                        content += '<div id="tab-performance" class="tab-content">';
                        if (data.performance_data) {
                            content += '<pre class="crash-performance-data">' + data.performance_data + '</pre>';
                        } else {
                            content += '<p>无性能数据</p>';
                        }
                        content += '</div>';
                        
                        $('#crash-details-content').html(content);
    ...
    public function ajax_get_crash_details() {
        if (!current_user_can('manage_options')) {
            wp_die('Unauthorized');
        }
        ...
        $report = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM $this->table_name WHERE id = %d", $report_id
        ));
        
        if ($report) {
            // 解析应用状态JSON
            $app_state = '';
            if (!empty($report->app_state_json)) {
                $app_state = $report->app_state_json;
            }
            ...
            wp_send_json_success(array(
                ...
                'context_logs' => $report->context_logs ?: '无上下文日志',
                'user_actions' => $report->user_actions ?: '无用户操作记录',
                'app_state' => $app_state,
                'performance_data' => $report->performance_data ?: '无性能数据'
            ));
        } else {
