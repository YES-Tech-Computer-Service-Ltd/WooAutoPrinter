...
class DefaultOrderPrintTemplate @Inject constructor(
    ...
    private fun generateOrderInfo(order: Order, config: PrinterConfig, templateConfig: TemplateConfig,
        ...
        if (templateConfig.showOrderNumber) {
            ...
            val styled = ThermalPrinterFormatter.formatStyledLine(
                ...
                bold = templateConfig.styleOrderInfoBold,
                large = templateConfig.styleOrderInfoLarge,
                alignment = 'L'
            )
            sb.append(styled)
        }
    ...
    private fun generateCustomerInfo(order: Order, config: PrinterConfig, templateConfig: TemplateConfig,
        ...
        if (templateConfig.showCustomerName && order.customerName.isNotEmpty()) {
            val line = ThermalPrinterFormatter.formatStyledLine(
                ...
                paperWidth = paperWidth,
                bold = templateConfig.styleCustomerInfoBold,
                large = templateConfig.styleCustomerInfoLarge,
                alignment = 'L'
            )
            sb.append(line)
            hasContent = true
    ...
    private fun generateItemDetails(order: Order, config: PrinterConfig, templateConfig: TemplateConfig,
        ...
        if (!kitchenStyle && templateConfig.showItemPrices) {
            val left = if (templateConfig.styleOrderContentBold) ThermalPrinterFormatter.formatBold("Item") else "Item"
            val rightBase = "Qty x Price"
            val right = if (templateConfig.styleOrderContentBold) ThermalPrinterFormatter.formatBold(rightBase) else rightBase
        ...
        order.items.forEach { item ->
            ...
            if (!kitchenStyle && templateConfig.showItemPrices) {
                ...
                val leftName = if (templateConfig.styleOrderContentLarge || templateConfig.styleOrderContentBold) {
                    // 使用中文大字体方案 + 可选加粗
                    val enlarged = ThermalPrinterFormatter.formatChineseLargeFont(name, paperWidth)
                    if (templateConfig.styleOrderContentBold) "<b>$enlarged</b>" else enlarged
                } else name
                val rightStyled = buildString {
                    if (templateConfig.styleOrderContentLarge) append("<h><w>")
                    if (templateConfig.styleOrderContentBold) append("<b>")
                    append(qtyPrice)
                    if (templateConfig.styleOrderContentBold) append("</b>")
                    if (templateConfig.styleOrderContentLarge) append("</w></h>")
                }
                ...
                val line = ThermalPrinterFormatter.formatStyledLine(
                    ...
                    paperWidth = paperWidth,
                    bold = templateConfig.styleOrderContentBold,
                    large = templateConfig.styleOrderContentLarge,
                    alignment = 'L'
                )
                sb.append(line)
            }
