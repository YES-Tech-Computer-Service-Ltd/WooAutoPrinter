...
fun TemplatePreview(
    ...
    // 纸张宽度切换状态（持久化于ViewModel，避免切 tab 被重置）
...
fun TemplateSettings(
    ...
    Column(
        ...
        if (config.showStoreInfo && hasStoreInfo) {
            ...
            // 样式开关（Chip）
            if (config.showStoreName && storeName.isNotBlank()) {
                StyleChipRow(
                    boldSelected = config.styleStoreNameBold,
                    largeSelected = config.styleStoreNameLarge,
                    onBoldChange = { onConfigChange(config.copy(styleStoreNameBold = it)) },
                    onLargeChange = { onConfigChange(config.copy(styleStoreNameLarge = it)) },
                    indented = true
                )
            }
            ...
            if (config.showStoreAddress && storeAddress.isNotBlank()) {
                StyleChipRow(
                    boldSelected = config.styleStoreAddressBold,
                    largeSelected = config.styleStoreAddressLarge,
                    onBoldChange = { onConfigChange(config.copy(styleStoreAddressBold = it)) },
                    onLargeChange = { onConfigChange(config.copy(styleStoreAddressLarge = it)) },
                    indented = true
                )
            }
            ...
            if (config.showStorePhone && storePhone.isNotBlank()) {
                StyleChipRow(
                    boldSelected = config.styleStorePhoneBold,
                    largeSelected = config.styleStorePhoneLarge,
                    onBoldChange = { onConfigChange(config.copy(styleStorePhoneBold = it)) },
                    onLargeChange = { onConfigChange(config.copy(styleStorePhoneLarge = it)) },
                    indented = true
                )
            }
        }
        ...
        if (config.showOrderInfo) {
            ...
            if (config.showOrderNumber) {
                StyleChipRow(
                    boldSelected = config.styleOrderNumberBold,
                    largeSelected = config.styleOrderNumberLarge,
                    onBoldChange = { onConfigChange(config.copy(styleOrderNumberBold = it)) },
                    onLargeChange = { onConfigChange(config.copy(styleOrderNumberLarge = it)) },
                    indented = true
                )
            }
            ...
            if (config.showOrderDate) {
                StyleChipRow(
                    boldSelected = config.styleOrderDateBold,
                    largeSelected = config.styleOrderDateLarge,
                    onBoldChange = { onConfigChange(config.copy(styleOrderDateBold = it)) },
                    onLargeChange = { onConfigChange(config.copy(styleOrderDateLarge = it)) },
                    indented = true
                )
            }
        }
        ...
        if (config.showOrderContent) {
            ...
            if (config.showItemDetails && config.showItemPrices) {
                StyleChipRow(
                    boldSelected = config.styleQtyPriceBold,
                    largeSelected = config.styleQtyPriceLarge,
                    onBoldChange = { onConfigChange(config.copy(styleQtyPriceBold = it)) },
                    onLargeChange = { onConfigChange(config.copy(styleQtyPriceLarge = it)) },
                    indented = true
                )
            }
        ...
        if (config.showPaymentInfo) {
            StyleChipRow(
                boldSelected = config.stylePaymentMethodBold,
                largeSelected = config.stylePaymentMethodLarge,
                onBoldChange = { onConfigChange(config.copy(stylePaymentMethodBold = it)) },
                onLargeChange = { onConfigChange(config.copy(stylePaymentMethodLarge = it)) }
            )
        }
        ...
        if (config.showFooter) {
            StyleChipRow(
                boldSelected = config.styleFooterBold,
                largeSelected = config.styleFooterLarge,
                onBoldChange = { onConfigChange(config.copy(styleFooterBold = it)) },
                onLargeChange = { onConfigChange(config.copy(styleFooterLarge = it)) }
            )
        }
...
private fun StyleChipRow(
    ...
    Row(
        modifier = Modifier
            .fillMaxWidth()
            .padding(start = if (indented) 32.dp else 0.dp, top = 0.dp, bottom = 8.dp),
        verticalAlignment = Alignment.CenterVertically
    ) {
        androidx.compose.material3.FilterChip(
            selected = boldSelected,
            onClick = { onBoldChange(!boldSelected) },
            label = { Text("Bold") }
        )
        Spacer(modifier = Modifier.width(8.dp))
        androidx.compose.material3.FilterChip(
            selected = largeSelected,
            onClick = { onLargeChange(!largeSelected) },
            label = { Text("Large") }
        )
    }
}
