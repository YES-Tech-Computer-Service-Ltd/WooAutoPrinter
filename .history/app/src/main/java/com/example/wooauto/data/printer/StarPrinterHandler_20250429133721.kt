...
class StarPrinterHandler(private val context: Context) {
    companion object {
        ...
        // Star TSP100III专用命令集
        private val TSP100_INIT = byteArrayOf(
        ...
        private val TSP100_CLEAR = byteArrayOf(0x18).toByteString()  // CAN - 清除缓冲区

        private val TSP100_ALIGN_LEFT = byteArrayOf(0x1B, 0x61, 0x00).toByteString()    // ESC a 0
        private val TSP100_ALIGN_CENTER = byteArrayOf(0x1B, 0x61, 0x01).toByteString()  // ESC a 1
        private val TSP100_ALIGN_RIGHT = byteArrayOf(0x1B, 0x61, 0x02).toByteString()   // ESC a 2
        
        private val TSP100_BOLD_ON = byteArrayOf(0x1B, 0x45, 0x01).toByteString()   // ESC E 1
        private val TSP100_BOLD_OFF = byteArrayOf(0x1B, 0x45, 0x00).toByteString()  // ESC E 0
        
        private val TSP100_LINE_SPACE = byteArrayOf(
            0x1B, 0x33, 0x28,  // ESC 3 40 - 设置行间距
            0x1B, 0x32         // ESC 2 - 设置默认行间距
        ).toByteString()

        private val TSP100_CUT = byteArrayOf(
            0x1B, 0x64, 0x03,  // ESC d 3 - 进纸并切纸
            0x1D, 0x56, 0x42, 0x00  // GS V B 0 - 切纸（带预留）
        ).toByteString()
    ...
    suspend fun printWithTSP100Commands(content: String): Boolean {
        ...
        try {
            ...
            for (line in lines) {
                when {
                    line.startsWith("[C]") -> {
                        formattedContent.append(TSP100_ALIGN_CENTER)
                        formattedContent.append(processLine(line.substring(3)))
                    }
                    line.startsWith("[R]") -> {
                        formattedContent.append(TSP100_ALIGN_RIGHT)
                        formattedContent.append(processLine(line.substring(3)))
                    }
                    line.startsWith("[L]") -> {
                        formattedContent.append(TSP100_ALIGN_LEFT)
                        formattedContent.append(processLine(line.substring(3)))
                    }
                    else -> {
                        formattedContent.append(TSP100_ALIGN_LEFT)
    ...
    private fun processLine(line: String): String {
        return line.replace("<b>", TSP100_BOLD_ON)
                   .replace("</b>", TSP100_BOLD_OFF)
    }
