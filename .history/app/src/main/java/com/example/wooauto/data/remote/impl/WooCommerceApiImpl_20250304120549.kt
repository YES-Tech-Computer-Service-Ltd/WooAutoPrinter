...
class WooCommerceApiImpl(
    ...
    private suspend inline fun <reified T> executeGetRequest(endpoint: String, queryParams: Map<String, String> = emptyMap()): T {
        return withContext(Dispatchers.IO) {
            try {
                ...
                // 添加其他查询参数 - 记录每个添加的参数
                Log.d("WooCommerceApiImpl", "【参数构建】开始添加查询参数:")
                queryParams.forEach { (key, value) ->
                    // 记录每个参数的添加
                    Log.d("WooCommerceApiImpl", "【参数构建】添加: $key=$value")
    ...
    override suspend fun getOrders(page: Int, perPage: Int, status: String?): List<OrderDto> {
        ...
        if (!status.isNullOrBlank()) {
            // 确保状态值不包含方括号等特殊字符，这可能导致REST API解析错误
            // WooCommerce API期望状态参数是简单的字符串值
            val cleanStatus = status.trim()
            queryParams["status"] = cleanStatus
            Log.d("WooCommerceApiImpl", "【状态请求】获取订单 - 状态过滤: '$cleanStatus'")
        } else {
            Log.d("WooCommerceApiImpl", "【状态请求】获取所有状态的订单")
        }
        
        // 打印最终的请求参数
        Log.d("WooCommerceApiImpl", "【状态请求】最终参数: $queryParams")
        ...
        try {
            ...
            addAuthParams(urlBuilder) // 添加认证参数
            queryParams.forEach { (key, value) ->
                urlBuilder.addQueryParameter(key, value)
            }
            Log.e("WooCommerceApiImpl", "失败的请求URL: ${urlBuilder.build()}")
            throw e
        }
    }
