...
object OrderMapper {
    ...
    private fun parseFeeLines(entity: OrderEntity): List<com.example.wooauto.domain.models.FeeLine> {
        ...
        // 关键修复：确保从不同来源获取的值按优先级合并
        val effectiveDeliveryFee = woofoodInfo?.deliveryFee.takeIf { !it.isNullOrEmpty() && it != "0.00" } 
                                  ?: deliveryFeeFromApi.takeIf { !it.isNullOrEmpty() && it != "0.00" }
                                  ?: "0.00"
        
        val effectiveTip = woofoodInfo?.tip.takeIf { !it.isNullOrEmpty() && it != "0.00" }
                          ?: tipFromApi.takeIf { !it.isNullOrEmpty() && it != "0.00" }
                          ?: "0.00"
        
        println("【最终值】订单#${entity.number} 最终使用的配送费: $effectiveDeliveryFee, 小费: $effectiveTip")
    ...
    private fun createOrderWithUpdatedWooFoodInfo(
        ...
        // 如果是外卖订单，检查是否有配送费
        val isDelivery = updatedWooFoodInfo?.isDelivery ?: false
