...
class OrderRepositoryImpl @Inject constructor(
    ...
    override suspend fun markOrderAsPrinted(orderId: Long): Boolean = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val updatedList = cachedOrders.map { 
                if (it.id == orderId) {
                    val updatedOrder = it.copy(isPrinted = true)
                    Log.d("OrderRepositoryImpl", "已更新缓存中订单 ${it.id} 的打印状态：${it.isPrinted} -> true")
                    updatedOrder
                } else {
                    it
                }
            ...
            // 检查是否在缓存中找到并更新了订单
            val found = updatedList.any { it.id == orderId && it.isPrinted }
            if (!found) {
