...
class WooAutoApplication : Application(), WorkConfig.Provider {
    ...
    override fun attachBaseContext(base: Context) {
        // 在创建 Application 之前初始化 PreferencesManager
        val tempPrefsManager = PreferencesManager(base)
        
        // 使用 runBlocking 获取语言设置
    ...
    private fun scheduleOrderPolling() {
        runBlocking {
            // 只在有 API 凭证时调度
            val apiKey = preferencesManager.apiKey.value
            val apiSecret = preferencesManager.apiSecret.value
            val pollingInterval = preferencesManager.pollingInterval.value.toLong()
