...
class StarPrinterHandler(private val context: Context) {
    ...
    suspend fun printText(content: String): Boolean {
        ...
        try {
            ...
            // 检查是否是TSP100系列
            if (isTSP100Series) {
                Log.d(TAG, "$logPrefix 使用TSP100行模式打印")
                return printWithLineMode(content)
            }
    ...
    suspend fun printWithRasterMode(content: String): Boolean {
        ...
        try {
            ...
            // 添加打印开始标记
            formattedContent.append("\u001b\u001d\u0054\u0001")  // ESC GS T 1 - 开始打印任务
            ...
            for (line in lines) {
                ...
                // 每行结束添加换行和刷新缓冲区命令
                formattedContent.append("\n")
                formattedContent.append("\u001b\u001d\u0046\u0001")  // ESC GS F 1 - 刷新打印缓冲区
            }
            ...
            for (i in contentBytes.indices step chunkSize) {
                ...
                val chunk = String(contentBytes.slice(i until end).toByteArray())
                
                currentPrinter?.printAsync(chunk)?.await()
                delay(200)  // 每个数据块之间添加延时
            ...
            // 6. 结束打印任务
            Log.d(TAG, "$logPrefix 结束打印任务")
            currentPrinter?.printAsync("\u001b\u001d\u0054\u0000")?.await()  // ESC GS T 0 - 结束打印任务
            delay(300)

            // 7. 走纸和切纸
            Log.d(TAG, "$logPrefix 走纸和切纸")
            currentPrinter?.printAsync("\n\n\n$RASTER_MODE_CUT")?.await()
            delay(500)
    ...
    private fun processRasterLine(line: String): String {
        ...
        // 处理加粗标签
        if (line.contains("<b>")) {
            processedLine = processedLine.replace("<b>", RASTER_MODE_BOLD_ON)
            processedLine = processedLine.replace("</b>", RASTER_MODE_BOLD_OFF)
        }
        
        // 添加中文字符处理
        if (processedLine.any { it.code > 127 }) {
            processedLine = "\u001b\u001d\u0074\u0001$processedLine"  // ESC GS t 1 - 设置中文编码
        }
