...
class OrderMapper @Inject constructor(
    ...
    fun mapResponseToEntity(response: OrderResponse): OrderEntity {
        // 检查是否使用WooCommerce Food插件
        val useWooCommerceFood = runBlocking {
            wooCommerceConfig.useWooCommerceFood.first()
        }
        
        // 处理shipping address，在WooFood插件模式下，如果shipping为空则使用billing
        val finalShippingAddress = if (useWooCommerceFood) {
            val shippingAddress = formatAddress(response.shipping)
            val billingAddress = formatAddress(response.billing)
            
            // 如果shipping address为空，或者与billing address相同，
            // 并且是WooFood插件模式，则确保使用billing address
            if (shippingAddress.isBlank() || isEmptyShippingAddress(response.shipping)) {
                Log.d("OrderMapper", "WooFood模式: shipping address为空，使用billing address作为配送地址")
                billingAddress
            } else {
                shippingAddress
            }
        ...
        val customerNote = buildString {
            if (response.customerNote?.isNotEmpty() == true) {
                append("客户备注: ${response.customerNote}\n")
            }
            
            // 在WooFood模式下，如果没有有效的shipping address，在备注中记录使用了billing address
            if (useWooCommerceFood && (isEmptyShippingAddress(response.shipping) || formatAddress(response.shipping).isBlank())) {
                append("【WooFood配送地址】使用billing address作为配送地址\n")
            }
            ...
            response.metaData.forEach { meta ->
                when (meta.key) {
                    "exwfood_order_method" -> {
                        ...
                        append("exwfood_order_method: $orderMethod\n")
                    }
                    "exwfood_time_deli" -> {
                        val orderTime = meta.value.toString()
                        append("exwfood_time_deli: $orderTime\n")
                    }
                    else -> {
            ...
            if (tipAmount == "0.00" || deliveryFeeAmount == "0.00") {
                response.feeLines?.forEach { fee ->
                    ...
                    if (tipAmount == "0.00" && (
                        ...
                        tipAmount = fee.total
                        append("从feeLines直接提取的小费: $${fee.total}\n")
                    }
                    // 检查是否包含配送费关键词
                        ...
                        append("从feeLines直接提取的配送费: $${fee.total}\n")
                    }
                }
            }
            
            // 添加最终提取到的值
            append("API最终提取的小费: $$tipAmount\n")
            append("API最终提取的配送费: $$deliveryFeeAmount\n")
        }
        ...
        return OrderEntity(
            ...
            shippingAddress = finalShippingAddress, // 使用处理过的shipping address
