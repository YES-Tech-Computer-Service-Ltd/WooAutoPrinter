...
class DefaultOrderPrintTemplate @Inject constructor(
        ...
        order.items.forEach { item ->
            ...
            if (item.options.isNotEmpty()) {
                item.options.forEach { option ->
                    val optionText = if (kitchenStyle) "${option.name}: ${option.value}" else "- ${option.name}: ${option.value}"
                    // 使用支持中文的缩进文本格式化
                    sb.append(ThermalPrinterFormatter.formatIndentedText(optionText, 1, paperWidth))
                }
            }
    ...
    private fun generateFooter(order: Order, config: PrinterConfig, templateConfig: TemplateConfig,
        ...
        if (templateConfig.showFooter && templateConfig.footerText.isNotBlank()) {
            ...
            // 使用支持中文的多行文本格式化方法，并设置为居中对齐
            // 这样可以处理自动换行和手动换行的情况
            sb.append(ThermalPrinterFormatter.formatMultilineText(templateConfig.footerText, paperWidth, 'C'))
            
            sb.append(ThermalPrinterFormatter.addEmptyLines(3)) // 添加空行便于撕纸
        } else {
            sb.append(ThermalPrinterFormatter.addEmptyLines(3))
        }
        ...
        return Order(
            ...
            items = listOf(
                OrderItem(
                    ...
                    image = "",
                    options = listOf(
                        ItemOption("配菜 Side dishes", "米饭、蔬菜、汤 Rice, Vegetables, Soup"),
                ...
                OrderItem(
                    ...
                    image = "",
                    options = listOf(
                        ItemOption("温度 Temperature", "冰镇 Cold"),
    ...
    private fun createDefaultTestOrder(): Order {
        ...
        return Order(
            ...
            isPrinted = false,
            notificationShown = false,
            notes = "默认打印机兼容性测试订单\nDefault printer test order for compatibility",
            woofoodInfo = WooFoodInfo(
                orderMethod = "Takeaway",
                deliveryTime = null,
                deliveryAddress = null,
