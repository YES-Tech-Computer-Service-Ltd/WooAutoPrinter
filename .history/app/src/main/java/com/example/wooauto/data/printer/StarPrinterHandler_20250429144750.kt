...
class StarPrinterHandler(private val context: Context) {
    ...
    suspend fun printTSP100DirectCommands(): Boolean {
        ...
        try {
            ...
            // 添加光栅模式测试
            if (lastConnectedModel.lowercase().contains("tsp100iii") || 
                lastConnectedModel.lowercase().contains("tsp143iii")) {
                Log.d(TAG, "$logPrefix 检测到TSP100III系列，添加光栅模式测试...")
                try {
                    val rasterTestContent = """
                        [C]*** TSP100III光栅模式测试 ***
                        [C]--------------------
                        [L]打印机型号: ${lastConnectedModel}
                        [L]测试模式: 光栅(Raster)模式
                        [L]时间: ${java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(java.util.Date())}
                        [C]--------------------
                        [L]测试项目:
                    ...
                    val result = printWithRasterMode(rasterTestContent)
                    Log.d(TAG, "$logPrefix 光栅模式测试结果: $result")
                } catch (e: Exception) {
                    Log.e(TAG, "$logPrefix 光栅模式测试失败: ${e.message}", e)
                }
            }
