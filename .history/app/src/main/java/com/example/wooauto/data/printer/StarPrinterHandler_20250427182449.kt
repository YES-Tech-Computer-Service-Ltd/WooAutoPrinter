        ...
        return try {
            ...
            withContext(Dispatchers.IO) {
                ...
                try {
                    ...
                    Log.d(TAG, "$logPrefix 状态详情 - 有纸: ${status?.hasPaper}, 有错误: ${status?.hasError}, " +
                         "在线: ${!status?.offline}, 盖子关闭: ${!status?.coverOpen}, " +
                         "收纸器满: ${status?.receiptPaperEmpty}, 错误状态: ${status?.errorStatus}")
                } catch (e: Exception) {
            ...
            try {
                withContext(Dispatchers.IO) {
                    ...
                    Log.d(TAG, "$logPrefix 打印机信息: 型号=${info?.model ?: "未知"}, " +
                          "固件=${info?.firmwareVersion ?: "未知"}, " +
                          "DSW=${info?.dSWSettings ?: "未知"}")
                }
            } catch (e: Exception) {
        ...
        try {
            ...
            try {
                ...
                if (status?.hasError == true) {
                    Log.e(TAG, "$logPrefix 打印后状态显示有错误: ${status}, 错误状态: ${status.errorStatus}")
                    return false
                }
            } catch (e: Exception) {
