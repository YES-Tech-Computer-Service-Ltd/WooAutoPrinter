...
object OrderMapper {
    ...
    private fun parseFeeLines(entity: OrderEntity): List<com.example.wooauto.domain.models.FeeLine> {
        ...
        // 直接使用parseWooFoodInfo中提取的配送费和小费值
        val effectiveDeliveryFee = woofoodInfo?.deliveryFee ?: "0.00"
        val effectiveTip = woofoodInfo?.tip ?: "0.00"
        
        println("【FEES-DIRECT】订单#${entity.number} - 使用woofoodInfo提供的配送费: $effectiveDeliveryFee, 小费: $effectiveTip")
        
        // 重要：始终检查woofoodInfo中的isDelivery标志
        if (isDelivery) {
            // 对于配送订单，始终添加配送费（即使金额为0）
            ...
            println("【FEES-ADD】订单#${entity.number} - 添加配送费: $effectiveDeliveryFee")
        }
        ...
        println("【FEES-ADD】订单#${entity.number} - 添加小费: $effectiveTip")
        ...
        // 打印最终的费用行，帮助调试
        println("【FEES-FINAL】订单#${entity.number} 最终feeLines集合:")
        feeLines.forEach { feeLine ->
            println("  - ${feeLine.name}: ${feeLine.total}")
        }
