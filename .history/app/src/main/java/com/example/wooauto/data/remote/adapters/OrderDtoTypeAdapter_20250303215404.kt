...
class OrderDtoTypeAdapter : TypeAdapter<OrderDto>() {
    // 创建元数据处理器注册表
    private val metadataRegistry = MetadataProcessorRegistry.getInstance()
    ...
    private fun readLineItems(reader: JsonReader): List<LineItemDto> {
        ...
        while (reader.hasNext()) {
            ...
            var metaData = emptyList<com.example.wooauto.data.remote.dto.MetaDataDto>()
            var image: com.example.wooauto.data.remote.dto.ImageDto? = null
    ...
    private fun readImage(reader: JsonReader): com.example.wooauto.data.remote.dto.ImageDto {
        var id: Long? = null
        var src: String? = null
        ...
        return com.example.wooauto.data.remote.dto.ImageDto(id, src)
    }
} 