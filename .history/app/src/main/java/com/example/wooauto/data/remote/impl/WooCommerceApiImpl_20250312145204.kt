...
class WooCommerceApiImpl(
    ...
    private suspend inline fun <reified T> executeGetRequest(endpoint: String, queryParams: Map<String, String> = emptyMap()): T {
        ...
        if (isAndroid7 && isOrderRequest) {
            ...
            // 对于Android 7上的订单API，尝试使用替代方法直接获取数据 - 最终手段
            if (endpoint == "orders" && currentRetry == 0) {
                try {
                    Log.d("WooCommerceApiImpl", "【Android 7特殊方案】尝试使用HTTP直接请求获取订单")
                    ...
                    try {
                        ...
                        if (responseCode == 200) {
                            ...
                            Log.d("WooCommerceApiImpl", "【Android 7特殊方案】成功获取响应，大小: ${responseBody.length}字节，耗时: ${duration}ms")
                ...
                Log.d("WooCommerceApiImpl", "【Android 7特殊方案】无法通过HTTP直接请求获取数据，回退到标准方法")
            }
        }
