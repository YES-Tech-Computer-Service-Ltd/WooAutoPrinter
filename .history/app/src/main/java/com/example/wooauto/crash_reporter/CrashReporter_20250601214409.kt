...
class CrashReporter private constructor(
    ...
    private fun collectCrashData(
        ...
        // 收集上下文信息（根据配置限制）
        val contextLogs = ContextLogger.getContextLogsAsString(config.dataSizeConfig.maxContextLogs)
        val userActions = ContextLogger.getUserActionsAsString(config.dataSizeConfig.maxUserActions)
        val appStateSummary = AppStateCollector.getAppStateSummary()
        val performanceData = PerformanceMonitor.getPerformanceSummary()
        
        // 记录崩溃事件到上下文日志
        ContextLogger.e("CrashReporter", "应用崩溃: ${throwable.javaClass.simpleName}", throwable)
        
        // 创建原始崩溃数据
        val rawCrashData = CrashData(
        ...
        // 优化数据大小
        val optimizedData = CrashDataOptimizer.optimizeCrashData(rawCrashData, config.dataSizeConfig)
        
        // 记录优化统计
        if (config.enableDebugLogs) {
            val stats = CrashDataOptimizer.getDataSizeStats(rawCrashData)
            val optimizedStats = CrashDataOptimizer.getDataSizeStats(optimizedData)
            Log.d(TAG, "数据优化: 原始大小 ${stats["total"]}B -> 优化后 ${optimizedStats["total"]}B")
        }
    ...
    private fun saveCrashDataLocally(crashData: CrashData) {
        try {
            ...
            var jsonData = crashData.toJson()
            
            // 如果启用压缩且数据较大，则压缩保存
            if (config.dataSizeConfig.enableCompression && jsonData.length > 5000) {
                val compressed = CrashDataOptimizer.compressText(jsonData)
                if (compressed != null && compressed.length < jsonData.length) {
                    jsonData = compressed
                    Log.d(TAG, "崩溃数据已压缩保存: ${fileName}")
                }
            }
            
            file.writeText(jsonData)
            
            Log.d(TAG, "Crash data saved: $fileName (${jsonData.length} bytes)")
        } catch (e: Exception) {
...
data class CrashReporterConfig(
    ...
    val dataSizeConfig: DataSizeConfig
) 