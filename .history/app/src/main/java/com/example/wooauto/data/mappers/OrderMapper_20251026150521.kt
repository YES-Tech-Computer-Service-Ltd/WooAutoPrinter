...
object OrderMapper {
    ...
    fun mapResponseToEntity(response: OrderResponse): OrderEntity {
        if (BuildConfig.DEBUG) {
            try {
                val metaDump = response.metaData.joinToString(separator = "; ") { "${it.key}=${it.value}" }
                Log.d(
                    "OrderDebug",
                    "API OrderResponse -> id=${response.id}, number=${response.number}, dateCreated=${response.dateCreated}, orderMethod=${response.orderMethod}, deliveryDate=${response.deliveryDate}, deliveryTime=${response.deliveryTime}, tip=${response.tip}, feeLines=${response.feeLines?.size ?: 0}; meta: ${metaDump}"
                )
            } catch (_: Exception) { }
        }
        // 提取元数据信息并构建元数据字符串
        ...
        if (BuildConfig.DEBUG) {
            try {
                ...
                Log.d(
                    "OrderDebug",
                    "Mapped OrderEntity -> id=${entity.id}, number=${entity.number}, dateCreated=${df.format(Date(entity.dateCreated))}, customerNote.length=${entity.customerNote.length}, shippingAddressEmpty=${entity.shippingAddress.isEmpty()}"
                )
            } catch (_: Exception) { }
        }
        return entity
    ...
    fun mapEntityToDomain(entity: OrderEntity): Order {
        ...
        if (BuildConfig.DEBUG) {
            try {
                ...
                Log.d(
                    "OrderDebug",
                    "Domain Order -> id=${domainOrder.id}, number=${domainOrder.number}, dateCreated=${df.format(domainOrder.dateCreated)}, woofoodInfo=${domainOrder.woofoodInfo}, feeLines=${domainOrder.feeLines.size}"
                )
            } catch (_: Exception) { }
        }
        return domainOrder
