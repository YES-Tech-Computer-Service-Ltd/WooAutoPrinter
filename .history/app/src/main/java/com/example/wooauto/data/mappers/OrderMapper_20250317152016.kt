    ...
    fun mapResponseToEntity(response: OrderResponse): OrderEntity {
        ...
        response.meta_data?.forEach { meta ->
            when (meta.key) {
                ...
                "exwfood_time_deli" -> {
                    orderTime = meta.value.toString()
                    metadataBuilder.appendLine("exwfood_time_deli: $orderTime")
                }
                // 添加其他可能的元数据
                else -> {
                    // 如果元数据键值有用，添加到构建器中
                    if (!meta.key.isNullOrEmpty() && meta.value != null) {
        ...
        response.fee_lines?.forEach { fee ->
            ...
            if (feeName.contains("tip") || 
                ...
                tipInfo = "Show Your Appreciation: $${fee.amount}"
                metadataBuilder.appendLine(tipInfo)
            }
        }
        ...
        return OrderEntity(
            ...
            customerName = buildCustomerName(response),
            billingAddress = buildBillingAddress(response.billing),
            shippingAddress = buildShippingAddress(response.shipping),
            contactInfo = buildContactInfo(response.billing),
            lineItems = mapLineItems(response.line_items)
        )
    }
