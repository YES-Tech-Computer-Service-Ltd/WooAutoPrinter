            ...
            try {
                ...
                val finalOrders = if (status != null) {
                    ...
                    val filteredOrders = orders.filter { 
                        ...
                        val matches = orderStatus.equals(status) || 
                                       (statusMap[status]?.equals(orderStatus) == true) || 
                                       (statusMap[orderStatus]?.equals(status) == true)
    ...
    override fun getOrdersByStatusFlow(status: String): Flow<List<Order>> {
        ...
        return _ordersFlow.asStateFlow().map { allOrders ->
            ...
            val filteredOrders = allOrders.filter { order ->
                ...
                // 2. 订单状态的中文名与请求的状态匹配
                // 3. 订单状态的英文名与请求的状态匹配
                val matchesDirect = order.status.equals(status)
                val matchesViaChinese = statusMap[order.status].equals(status)
                val matchesViaEnglish = statusMap[status].equals(order.status)
