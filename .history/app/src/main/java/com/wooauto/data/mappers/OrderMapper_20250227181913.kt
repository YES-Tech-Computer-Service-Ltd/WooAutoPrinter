...
object OrderMapper {
    ...
    fun mapResponseToEntity(response: OrderResponse): OrderEntity {
        // 从元数据中提取配送信息
        val metaData = response.metaData ?: emptyList()

        // 提取订单方式 (delivery/pickup)
        val orderMethod = metaData.find { it.key == "_exwfood_order_method" }?.value?.toString() ?: ""

        // 提取配送日期
        val deliveryDate = metaData.find { it.key == "_exwfood_date_deli" }?.value?.toString() ?: ""

        // 提取配送时间
        val deliveryTime = metaData.find { it.key == "_exwfood_time_deli" }?.value?.toString() ?: ""

        // 查找小费相关元数据（如果存在）
        // 注意：这里可能需要根据你的实际元数据字段名称调整
        val tip = findTipFromLineItems(response)

        // 提取配送费用
        val deliveryFee = findDeliveryFeeFromLineItems(response)

        // 对于配送地址，如果shipping地址为空但是是配送订单，则使用billing地址
        val shippingAddress = if (isEmptyShippingAddress(response.shipping) && orderMethod.equals("delivery", ignoreCase = true)) {
